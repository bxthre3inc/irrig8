...
Before specifying the Recursive Spawning Protocol, we must precisely characterize the failure mode it targets: \emph{autonomous drift} — the condition in which an agent continues to operate with no traceable authorization path to a human principal. This section defines the formal vocabulary for that characterization, establishes the two constituent sub-modes of drift, identifies the three structural conditions that enable them, catalogs four concrete failure modes, proves that depth limits alone are insufficient to prevent drift, and concludes with a compact formal system model used throughout the remainder of this paper.
...
Orphaning severs the delegation chain at the parent node. The agent $n$ persists in operational state but is structurally disconnected from its chain of authorization. The disconnection is the key property: an orphaned agent may continue executing its event loop indefinitely, unaware that its parent no longer exists.
...
A drifted agent may be orphaned or may have a living parent whose own chain does not reach any human principal. Drift is a property of the chain's \emph{origin}, not of the chain's intermediate connectivity. A node whose parent is alive and responsive can still be drifted if the chain above the parent traces to a non-human root.
...
The six possible combinations of the binary sub-modes (orphaned, drifted) and the operating state are given in Table~\ref{tab:drift-submodes}.
