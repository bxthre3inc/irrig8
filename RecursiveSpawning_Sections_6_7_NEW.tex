...
The Recursive Spawning Protocol is not a layer added to an existing agent architecture. It is a protocol that \emph{activates} the three-layer BX3 (Behavior, Execution, and eXchange) architecture at every spawn event, relying on all three layers simultaneously to enforce its guarantees. The Purpose Layer provides the accountability anchor; the Bounds Engine provides the structural depth enforcement; the Fact Layer provides the forensic ledger. No single layer is sufficient; all three are structurally necessary. The integration is a structural necessity, not a mapping of convenience.

% -------------------------------------------------------------------------
\subsection{Purpose Layer: Accountability Anchor}

The Purpose Layer is the singular accountability anchor in the BX3 architecture. Every agent---including the root agent at the head of any spawn chain---is initialized with a \textbf{Purpose Anchor}: a cryptographically signed record declaring the agent's intended operational scope, the authorized principal who chartered its creation, and the terminal objective it is chartered to pursue. The Purpose Anchor is immutable post-spawn. No actor---including the agent itself, any ancestor, or any administrative process---may revise a Purpose Anchor after the agent enters operational state. Any attempted revision is rejected by the Fact Layer pre-commit hook before it can take effect.

The Purpose Layer serves two distinct roles in Recursive Spawning. First, it defines the operational range $R(n)$ for every node $n$ at provisioning. This range is the binding constraint that the Bounds Engine enforces at every spawn event. When a node spawns a child, the Purpose Layer's definition of $R(n)$ propagates downward as an upper bound on $R(\text{child})$: the child cannot receive authority its parent does not possess. This is the \textbf{authority monotonicity} property. Second, the Purpose Layer is the structural terminus for the escalation chain. Every node $n$ is assigned a human anchor $h(n)$ at provisioning---a specific human principal who is the ultimate accountable party for all actions under $R(n)$. The chain does not terminate at a depth limit imposed by operational convenience; it terminates logically at the Purpose Layer, which represents the point of original human authorization. An agent whose purpose chain cannot be traced to a human principal is, by architectural definition, unauthorized and must not enter operational state.
...
We show that three mutually reinforcing structural properties jointly preclude each necessary condition for drift. Drift requires that at least one of three conditions holds: (i) the parent pointer is modified post-spawn, (ii) the purpose clause diverges from its authorized scope at runtime, or (iii) the chain fails to reach a human principal.

\textit{Pathway 1: Parent-pointer modification.} The parent pointer $\alpha(c) = p$ is established by the Fact Layer at spawn time via the \texttt{child-provisioned} event and is structurally immutable thereafter. The Fact Layer exposes only an \texttt{append} operation; no \texttt{overwrite} or \texttt{redact} operation exists in the write interface. Any attempt to redirect $\alpha(c)$ to a different parent $p'$ requires appending a new entry, which the verification protocol rejects because the appended entry's hash does not match the child's already-sealed identity credential. Modification is therefore cryptographically impossible, not merely prohibited by policy.

\textit{Pathway 2: Purpose clause drift.} The Purpose Layer enforces that every spawned agent's purpose clause is a strict descendant refinement of its parent's purpose at spawn time (Invariant PL-V1: $R(c) \subset R(p)$). At execution time, the Truth Gate verifies that every action is consistent with the sealed purpose. Any action that would constitute drift---any action not reconcilable with the sub-objective authorized by the parent and ultimately by $p$---fails the Truth Gate and is not sealed. Because the purpose clause is frozen at spawn time and the Truth Gate rejects inconsistent actions, the agent cannot drift at runtime regardless of its internal reasoning.
...
The three pathways together exhaust the necessary conditions for drift. Because all three are structurally precluded, no agent in chain $C$ can exhibit drift with respect to $p$'s purpose. $\square$
\end{proof}
