# VPC Affiliate Program — Program Design
> Harvest — Affiliate & Partner Marketing Lead
> **Draft Date:** 2026-03-30

---

## Overview

Valley Players Club affiliate program is the primary acquisition channel. This doc defines the affiliate structure, commission tiers, compliance requirements, and operational workflow.

---

## Affiliate Types

### Tier 1: Affiliate Managers
**Profile:** Individual or company managing multiple affiliate sites/players
**Commission:** 30% revenue share
**Typical Volume:** Medium (10-50 players/month)
**Compliance:** Standard gaming affiliate disclosure, state-specific licensing

### Tier 2: Casino Aggregators
**Profile:** Platforms aggregating multiple sweepstakes/casino offerings
**Commission:** 25% revenue share
**Typical Volume:** High (50-500 players/month)
**Compliance:** Master affiliate agreement, sub-affiliate terms

### Tier 3: Individual Promoters
**Profile:** Social media influencers, content creators
**Commission:** CPA model ($50-100 per active player)
**Typical Volume:** Low-medium
**Compliance:** FTC disclosure, platform terms

---

## Commission Structure

| Tier | Partner Type | Revenue Share | CPA | Notes |
|------|-------------|---------------|-----|-------|
| 1 | Affiliate managers | 30% | — | Recurring rev share |
| 2 | Casino aggregators | 25% | — | Recurring rev share |
| 3 | Individual promoters | — | $50-100/player | Per active player |

**Payment Terms:**
- Monthly reconciliation, 30-day payment lag
- Minimum payout threshold: $100
- Payment methods: Bank transfer, PayPal

---

## Compliance Requirements

| Requirement | Status |
|-------------|--------|
| Gaming license verification by state | TBD — VPC Agent |
| Affiliate agreement (terms of service) | TBD |
| FTC disclosure compliance | Required |
| Anti-spam compliance (CAN-SPAM, CASL) | Required |
| State-specific gaming disclosures | TBD per state |

---

## Affiliate Lifecycle

```
1. RECRUITMENT
   → Outbound: affiliate managers, aggregators
   → Inbound: affiliate application portal

2. ONBOARDING
   → Application review (24-48 hrs)
   → Agreement signing (digital)
   → Account setup + tracking link generation
   → Orientation + creative assets

3. ACTIVATION
   → Provide marketing assets (banners, links, copy)
   → Assign affiliate manager (for Tier 1/2)
   → Set performance baseline

4. OPTIMIZATION
   → Monthly performance review
   → Commission tier adjustments (quarterly)
   → Top performer recognition

5. PAYMENT
   → Monthly reconciliation
   → Invoice processing
   → Payment disbursement
```

---

## Tracking & Attribution

**Tracking Model:** Last-click attribution with 30-day window

**Key Metrics:**
- Registered users per affiliate
- First deposit rate
- Active players (30-day)
- Revenue per player
- Conversion rate

**Platform Requirements:**
- [ ] Affiliate portal (self-service dashboard)
- [ ] Unique tracking links per affiliate
- [ ] Real-time reporting
- [ ] Automated commission calculation
- [ ] Payment processing integration

---

## Top 5 Outreach Targets

*Research conducted 2026-04-03 — Market scan of sweepstakes/iGaming affiliate landscape*

| # | Target Type | Examples | Commission |
|---|-------------|----------|------------|
| 1 | **Sweepstakes affiliate managers** | Individual managers (LinkedIn: "affiliate manager" + sweepstakes keywords), portfolios of 10-50 players. Networks: xlWin Media, Offervault directories. | 30% rev share |
| 2 | **Casino aggregators** | Starcrown Partners, N1 Partners, Chipstars (note: most focus on real-money Curacao-licensed; sweepstakes-specific less mature) | 25% rev share |
| 3 | **CPA networks (sweepstakes-friendly)** | Algo-Affiliates, CPAGrip, Adscend Media — run sweepstakes DOI/SOI campaigns | CPA ($50-100/player) |
| 4 | **Social casino streamers** | Twitch/Kick streamers partnered with licensed social casino platforms | CPA model |
| 5 | **Content marketing affiliates** | SweepsKings, PlayUSA, Lineups.com — editorial/affiliate contacts at sweepstakes review publications | Rev share or CPA |

**Research sources:** AffPapa, RichAds blog, GammaStack sweepstakes marketing guide, SweepsKings company directory, xlWin Media sweepstakes guide.

**Compliance note:** Sweepstakes casinos operate under promotional sweepstakes frameworks (dual-currency: Gold Coins + Sweeps Coins). Generally do NOT require Curacao or state gaming licenses for affiliate operations — unlike real-money iGaming. FTC disclosure requirements and state-specific marketing regulations still apply. VPC Agent must confirm VPC's specific compliance posture.

---

## Next Steps

- [x] ~~Get VPC affiliate target list from Reseller (Channel)~~ ✅ **RESOLVED — Harvest conducted independent research (2026-04-03)**
- [ ] Confirm compliance requirements with VPC Agent — **OVERDUE since 2026-03-31**
- [ ] Begin outreach to affiliate manager targets (Top 5 identified)
- [ ] Design affiliate agreement template
- [ ] Source/build affiliate tracking platform

---

*Harvest — Affiliate & Partner Marketing Lead*