...
- AgentOS and Antigravity use stdio (no API key needed)
...
| **C: Peer-specific tokens** | AgentOS uses AgentOS token, Antigravity uses its own | Logical separation | Need token generation system |
...
4. Each peer (AgentOS, Antigravity) should have its own identity
