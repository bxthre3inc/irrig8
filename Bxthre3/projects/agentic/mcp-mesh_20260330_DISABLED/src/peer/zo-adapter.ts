 ...
 * - Serve tools to other peers (AgentOS tools, Antigravity tools)
 * - Consume tools from other peers
 * - Participate in context sync
 * - Publish/subscribe to events
 * 
 * This makes the mesh truly 3-way: Zo ↔ Antigravity ↔ AgentOS
 */
...
export class ZoMCPAdapter implements IDEAdapter {
  ...
  private registerZoTools(): void {
    ...
    // AgentOS delegation
    this.tools.set("zo_agentos_task", async (args) => {
      if (!this.mesh) return { content: [{ type: "text", text: "Mesh not connected" }] };
      const result = await this.mesh.callTool("agentos", args.tool as string, args.args as Record<string, unknown>);
      return { content: [{ type: "text", text: JSON.stringify(result) }] };
    });

    this.tools.set("zo_agentos_escalate", async (args) => {
      if (!this.mesh) return { content: [{ type: "text", text: "Mesh not connected" }] };
      const result = await this.mesh.callTool("agentos", "escalate", {
...
// ✅ AgentOS Adapter — wraps AgentOS as an MCP peer
