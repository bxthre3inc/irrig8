...
console.log("[AgentOS] Unified backend starting...");
...
// AgentOS Core
app.get("/agents", () => c.json({ agents: Array.from(agents.values()) }));
app.get("/inbox", () => c.json({ items: Array.from(inbox.values()).sort((a, b) => b.timestamp - a.timestamp) }));
...
console.log(`[AgentOS] Unified backend running on :${PORT}`);
console.log(`[AgentOS] Endpoints:`);
console.log(`  GET  /status        - Mesh + AgentOS status`);
...
console.log(`[AgentOS] Worker ID: ${WORKER_ID}`);
