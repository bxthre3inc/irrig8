...
\texttt{Bailout\-Protocol.fire()}.  This exclusivity is architecturally enforced:
...
does not block actions directly.  Upon detecting a trigger condition, it generates a
\texttt{BailoutTrigger} record and returns control to the Bailout Protocol
...
when $\text{confidence} < \theta_{\text{safety}}$ for a projected outcome, the
engine fires a \texttt{'safety\_envelope\_predicted'} trigger preemptively — before
the violation actually occurs.  This anticipatory triggering is the basis of the
protocol's preventive rather than merely reactive posture.  A bail-out fired at
$\text{confidence} = 0.91$ indicates that the Bounds Engine projected a safety-envelope
violation with 91\% simulated fidelity.  This makes bail-out thresholds auditable
and tunable without code changes — the threshold $\theta_{\text{safety}}$ is a
configuration parameter, not a hardcoded constant.
...
Protocol context are threefold:
...
\end{verbatim}
  ...
  \item \textbf{Audit trail enforcement.}  Every transition through the Sandbox Gate
        ...
        as a \texttt{FACT\_EVALUATION} entry in the forensic ledger, capturing the
        input decision, the verdict, the \texttt{evaluatorNodeId}, and the
        \texttt{evaluatedAt} timestamp.  This creates a complete, causally ordered
        record of every enforcement decision in the system.
...
\begin{itemize}
  ...
  \item \texttt{resolve(\{resolutionText, resolverId\})} — P7/P8 transition.
        ...
        entry is appended to the ledger.  Execution is unblocked only after this
        transition completes.
\end{itemize}

The state machine is formally a Fact Layer extension in the following precise sense:
every state transition generates a corresponding \texttt{LedgerEntry} in the
Forensic Ledger, and the plane designator (P1 through P9) carried by each ledger
entry is the primary key for forensic queries.  The state machine does not maintain
separate state — it is the Fact Layer's state vector expressed through the
\texttt{BailoutTrigger} record.  This formal alignment ensures that the state
machine's determinism is not an implementation choice but a structural consequence
of the Fact Layer's determinism invariant: given the same trigger event and the same
\texttt{propagationChain}, the protocol always produces the same sequence of state
transitions and the same ledger entries.
...
protocol emits a \texttt{TRIGGER\_EXPIRED} ledger entry.  The failure policy that
follows — whether the system halts, falls back to a default action, or continues
in a degraded mode — is a domain-specific parameter passed to the protocol at
initialisation and lies outside the protocol definition itself.

% ----------------------------------------------------------------
\subsection{Forensic Ledger: SHA-256 Chained Recording}
% ----------------------------------------------------------------

The \texttt{ForensicLedger} (\texttt{ForensicLedger.ts}) is a write-once append-only
log that records every state transition, trigger event, and human resolution
decision in the Bailout Protocol.  It is the evidentiary substrate of the BX3
accountability guarantee — the artifact that makes the upstream commitment
machine-verifiable.
...
interface LedgerEntry {
  ...
  eventType:    string        // e.g., TRIGGER_FIRED, ESCALATED, RESOLVED
  payload:      Record<string,unknown>  // event-specific data
  previousSeal: string        // hash of the preceding entry (or 'GENESIS')
...
and rewrite all entries $k+1, k+2, \ldots, n$ to maintain consistency — a cost that
