...
\begin{enumerate}
    \itemsep0em
    \item \textbf{Spawn authorization lookup:} Given node $c$, recover $(p, \sigma_c, t, \phi)$ from the ledger to establish the authorized parent and scope.
    \item \textbf{Chain reconstruction:} Reconstruct $\chi_{\mathcal{L}}(c) = (c_0^{\mathcal{L}}, c_1^{\mathcal{L}}, \ldots)$ from ledger entries and compare against the in-memory chain $\chi(c)$. Any mismatch flags an integrity violation and triggers the Bailout Protocol.
...
\textit{Case 2: $c$ acts on $p'$ without modifying $\alpha(c)$.} Any action taken on an unauthorized purpose produces observable state in the Fact Layer (sensor inputs, actuator commands, ledger entries). The forensic ledger records all such states. Divergence is therefore detectable by comparing observed behavior against the authorized scope $\sigma_c$ recorded at spawn. Since the divergence is detectable, condition (a) is violated. $\square$
\endproof

The proof sketch establishes that drift prevention emerges from the \textit{conjunction} of three properties: (i) immutable parent pointers prevent retroactive chain-editing, (ii) the forensic ledger detects behavioral divergence against authorized scope, and (iii) the chain terminates at a human who can exercise the Bailout Protocol. No single property suffices; all three are necessary.
