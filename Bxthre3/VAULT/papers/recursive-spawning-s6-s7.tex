...
The Bounds Engine provides constrained execution evaluation: it evaluates proposed actions against the Purpose-Layer-defined operating range and enforces the depth constraints that prevent unbounded chain growth. Critically, the Bounds Engine cannot exceed these constraints under any operational circumstances.
...
\medskip
\noindent\textbf{Pre-commit hook as enforcement mechanism.} The pre-commit hook is the mechanism that makes all other RSP constraints structural. At every spawn event, it verifies (a) $R(n_{i+1}) \subseteq R(n_i)$ and (b) $\mathrm{depth}(n_{i+1}) \leq D_{\mathrm{max}}$ before the spawn is recorded. Rejected spawns are logged; the Bounds Engine does not execute them; the protocol transitions to \texttt{ESCALATING} if the unmet condition persists. Without the pre-commit hook, the scope refinement constraint and depth bound are policy documentation — enforceable until an actor with sufficient privilege decides they are inconvenient. The Fact Layer is the structural enforcement layer that makes the policy effective.

% ----------------------------------------------------------
\subsection{Structural Minimality}
\label{sec:rs-minimality}

The three-layer mapping is minimal and sufficient. No proper subset of layers achieves RSP's correctness properties. Removing any single layer causes at least one guarantee to fail, as shown in Table~\ref{tab:layer-minimality}.
...
\medskip
The proof structure rests on three architecturally necessary conjuncts that jointly make drift structurally impossible. No single conjunct suffices; removing any one collapses the proof:

\begin{enumerate}
    \item[(1)] \textbf{Immutable parent pointer.} $\alpha(n)$ is established once at provisioning and is thereafter immutable under the Fact Layer write protocol. Scope inheritance is traceable to a unique authorized lineage — not a manipulable graph. Without an immutable parent pointer, a drifted node could re-parent itself to a compliant ancestor, severing the accountability path and propagating a falsified lineage to its descendants.

    \item[(2)] \textbf{Forensic ledger.} Every scope assignment is recorded in the append-only, hash-chained ledger. The ledger makes each step of the refinement chain an empirically verifiable, tamper-evident record. Without the forensic ledger, the drift prevention guarantee would rest on the current chain state alone — a drifted chain could have a compliant-looking structure at the moment of inspection while its historical record had been altered. With the ledger, the entire refinement history is verifiable at any future time.
...
With all three conjuncts present, drift is architecturally impossible — not statistically unlikely, not conventionally prevented, but structurally impossible as a matter of protocol enforcement at every spawn step.
