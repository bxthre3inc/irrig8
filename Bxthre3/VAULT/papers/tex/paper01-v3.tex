...
\author{
    \textbf{Jeremy Blaine Thompson Beebe}\\
    \textit{Independent Researcher}\\
    \textit{Bxthre3 Inc.}\\
    \href{https://orcid.org/0009-0009-2394-9714}{\texttt{https://orcid.org/0009-0009-2394-9714}}\\
    \texttt{bxthre3inc@gmail.com}\\
    \date{April 2026}
}
...
\begin{figure}[htbp]
\centering
\caption{The BX3 Framework: Three immutable functional layers with directional communication and the Bailout Protocol.}
\label{fig:bx3loop}
\vspace{0.4em}
\begin{tabular}{p{15.5cm}}
\toprule
\textbf{Layer 1: PURPOSE LAYER} \hfill \textit{Intent $\mid$ Judgment $\mid$ Accountability} \\
\midrule
\textbf{Layer 2: BOUNDS ENGINE} \hfill \textit{Bounded Reasoning $\mid$ Analysis $\mid$ Proposal (Limbless)} \\
\midrule
\textbf{Layer 3: FACT LAYER} \hfill \textit{Deterministic Enforcement $\mid$ Forensic Ledger} \\
\bottomrule
\end{tabular}
\vspace{0.6em}
\begin{tabular}{p{7.5cm}p{7.5cm}}
\toprule
\textbf{Direction $\downarrow$} & \textbf{Feedback / Escalation $\uparrow$} \\
\midrule
Goals, SLOs, Constraints & Ledger / Forensic Records \\
Proposals, Simulations & Bailout Triggers \\
Authorization Scope & Violation Signals \\
\bottomrule
\end{tabular}
\end{figure}
...
\section{Formal Properties and Theorems}
...
\subsection{Layer Postulates}

\paragraph{Postulate 1: Purpose Layer -- Accountability.}
\label{post:p1}
For any BX3 node $N$, there exists a unique accountable actor $A(N)$ satisfying: (1) $A(N)$ is a named legal entity; (2) every action by $N$ is attributable to $A(N)$ in the forensic Ledger; (3) $A(N)$ may delegate authority but accountability transfers only upon explicit acceptance by the delegate.

\paragraph{Postulate 2: Bounds Engine -- Limblessness.}
\label{post:p2}
For any BX3 node $N$, the Bounds Engine $BE(N)$ satisfies: (1) $BE(N)$ may propose actions but may not execute them; (2) $BE(N)$ has no direct write access to physical actuators; (3) all proposals from $BE(N)$ must pass through the Fact Layer gate before physical execution.

\paragraph{Postulate 3: Fact Layer -- Determinism.}
\label{post:p3}
For any BX3 node $N$, the Fact Layer $FL(N)$ satisfies: (1) for identical input state $S$ and identical Purpose directive $P$, $FL(N)$ produces identical output state $S'$; (2) $FL(N)$ is provably non-Turing-complete -- it computes bounded functions only; (3) $FL(N)$ cannot be modified by $BE(N)$ or any descendant of $N$.

\subsection{Main Theorems}

\begin{quote}
\textbf{Theorem 1: Layer Isolation Guarantee.} In a BX3-compliant system, a Logic Collision -- wherein the reasoning function and execution function occupy the same functional plane -- is architecturally impossible.
\end{quote}
\textit{Proof.} A Logic Collision requires the Bounds Engine $BE(N)$ and Fact Layer $FL(N)$ to share a functional plane. By Postulate 2, $BE(N)$ has no direct write access to physical actuators. By definition, $FL(N)$ is the sole interface to physical execution. Since $BE(N) \neq FL(N)$ by construction, and $BE(N)$ cannot access the physical plane except through $FL(N)$, the two layers are architecturally isolated. Therefore a Logic Collision cannot occur. $\square$

\begin{quote}
\textbf{Theorem 2: Upstream Accountability Guarantee.} For any BX3 node $N$, any condition $C$ that $N$ cannot locally resolve escalates to $A(N)$ within a bounded number of hops in the recursive spawning tree.
\end{quote}
\textit{Proof.} Node $N$ encountering unresolved $C$ fires a Trigger Package to its parent $P(N)$. If $P(N)$ can resolve $C$ within its Safety Envelope, the trigger closes. If not, $P(N)$ escalates to $P^2(N)$. The recursive spawning tree is finite and the Human Root $H$ is always reachable. Since the Bailout Protocol mandates escalation at each unresolved hop, $C$ reaches $A(N) = H$ within bounded steps. $\square$
...
\begin{figure}[htbp]
\centering
\caption{The BX3 Framework: Recursive Spawning with immutable parent Purpose pointers and local worksheet execution.}
\label{fig:recursive-spawn}
\vspace{0.4em}
\begin{tabular}{p{15.5cm}}
\toprule
\textbf{Human Purpose Layer (Level 0)} \hfill \textit{Accountability Anchor} \\
\midrule
Parent node spawns child via Worksheet OTA deployment \\
Each Worksheet: immutable parent Purpose hash + Safety Envelope + local Fact Layer config \\
Child executes independently when cloud connectivity is lost \\
\bottomrule
\end{tabular}
\vspace{0.5em}
\begin{tabular}{p{7.5cm}p{7.5cm}}
\toprule
\textbf{Parent $\to$ Child} & \textbf{Child Self-Contained} \\
\midrule
Worksheet (OTA, over-the-air) & Local sensor data processing \\
Immutable parent Purpose hash & Local Fact Layer enforcement \\
Safety Envelope boundaries & Bailout Protocol (local trigger) \\
\bottomrule
\end{tabular}
\end{figure}


 through whom all Bounds Engine insight must pass before it becomes Fact Layer logic. Accountability is preserved throughout the improvement process, and the Forensic Ledger records every proposed change and its authorization status.
...
\begin{center}
\begin{tabular}{p{2.4cm} p{1.5cm} p{1.5cm} p{1.5cm} p{1.2cm} p{1.2cm} p{1.4cm}}
\toprule
\textbf{Architecture} & \textbf{Sep.} & \textbf{Hum. Acct.} & \textbf{Det. Fact} & \textbf{Safety Env.} & \textbf{Audit} & \textbf{Dist. Auto.} \\
\midrule
BX3 Framework & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark & \checkmark \\
ROS/ROS2 & \checkmark & \texttimes & \texttimes & \texttimes & \texttimes & \checkmark \\
Kubernetes & \checkmark & \texttimes & \checkmark & \texttimes & partial & \checkmark \\
JADE Multi-Agent & \checkmark & \texttimes & \texttimes & \texttimes & \texttimes & \checkmark \\
Holochain & \checkmark & \texttimes & \texttimes & \texttimes & \checkmark & \checkmark \\
ISO/IEC 42001 & \checkmark & \checkmark & \texttimes & \texttimes & \texttimes & \texttimes \\
NIST AI RMF & \checkmark & \checkmark & \texttimes & \texttimes & \texttimes & \texttimes \\
\bottomrule
\end{tabular}
\end{center}
\vskip 0.4em
\noindent\textbf{Interpretation:} The BX3 Framework is the only architecture evaluated that simultaneously satisfies all six properties by architectural construction. Governance frameworks (ISO, NIST) lack deterministic enforcement; infrastructure frameworks (Kubernetes, ROS) lack accountability; agent frameworks (JADE, Holochain) lack formal safety envelopes.

\vskip 0.5em
\noindent\textbf{Note on ROS, Kubernetes, JADE, and Holochain:} ROS provides modular communication but no accountability layer. Kubernetes provides container orchestration with isolation but no bounds-engine concept. JADE provides agent messaging but no deterministic fact layer. Holochain provides cryptographic audit but no safety-envelope enforcement. Each provides partial coverage; BX3 integrates all six.
...
The BX3 Framework proposes a clarifying framework: the \textbf{BX3 Framework}. The framework organizes any complex system into three immutable functional layers --- the \textit{Purpose Layer}, the \textit{Bounds Engine}, and the \textit{Fact Layer} --- each defined by the properties it must maintain rather than by the type of actor that occupies it. Any actor capable of satisfying a layer's functional requirements may occupy that layer: human, AI system, mechanical process, institutional entity, or hybrid. What cannot vary are the properties themselves --- accountability in the Purpose Layer, boundedness in the Bounds Engine, and determinism in the Fact Layer.
...
\begin{itemize}
    \item \textbf{Human Root scalability:} As the recursive tree scales, the human accountability anchor may become a bottleneck. Distributed accountability mechanisms require further specification.
    \item \textbf{Legacy interoperability:} Retrofitting SCADA, PLCs, and proprietary SaaS to BX3 layer separation may require significant refactoring. The cost-benefit of such migration is an open empirical question.
    \item \textbf{Formal verification toolchain:} A standardized Safety Envelope specification language (analogous to TLA+) is planned as a companion specification.
    \item \textbf{Performance overhead:} Initial AgentOS benchmarks suggest 2--5ms per execution cycle -- acceptable for agricultural and enterprise applications but potentially problematic for hard real-time systems.
    \item \textbf{Cross-organizational deployments:} Multiple legal entities in the same BX3 system require additional accountability transfer protocols not yet specified.
\end{itemize}
...
\textbf{2. Technical Soundness (30\%):} Are the postulates (Accountability, Limblessness, Determinism) well-defined and internally consistent? Are the two theorems correctly derived from the postulates? Are there counterexamples?

\textbf{3. Clarity and Completeness (20\%):} Is the framework sufficiently specified to be implemented without additional clarification? Are the five pillars clearly distinguished? Is the actor-agnostic property consistently applied?

\textbf{4. Significance (20\%):} Does the framework address a genuine gap in AI architecture and governance practice? Are the cross-domain examples (legal, medical, biological, organizational) appropriate and well-chosen?
...
\begin{itemize}
    ...
    \item[ ] Theorem 1 (Layer Isolation) and Theorem 2 (Upstream Accountability) formally stated and proved
    \item[ ] Postulates 1--3 clearly labeled
    \item[ ] Five pillars clearly named and distinguished
    \item[ ] Actor-agnostic property stated explicitly in Section~\ref{sec:layers}
...
\textbf{Keywords:} BX3 Framework, Purpose Layer, Bounds Engine, Fact Layer, artificial intelligence, deterministic systems, software architecture, human-in-the-loop, upstream accountability, autonomous systems, AI governance, sociotechnical systems, agentic systems, edge computing
...
\textbf{Conflicts of Interest:} The author is affiliated with Bxthre3 Inc., a company developing commercial implementations of the BX3 Framework.
