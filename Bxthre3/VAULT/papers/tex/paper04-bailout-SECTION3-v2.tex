...
\noindent\textbf{Algorithm \ref{alg:escalation}.}
The following recursive procedure governs propagation from node $B$ to the human anchor $\text{owner}(B)$:

\begin{trivlist}\item[]\raggedright
\textbf{Escalate}$(T, B)$: initialize $\phi \leftarrow T.\phi$, $B_{\text{cur}} \leftarrow B$.\\[3pt]
\textbf{while} $B_{\text{cur}} \neq \texttt{null}$:\\[3pt]
\phantom{\textbf{while}}\quad set $\sigma_{B_{\text{cur}}} \leftarrow \text{ESCALATING}$; append $\langle B_{\text{cur}}, \tau_{\text{now}}, \texttt{DISPATCHED}\rangle$ to $\phi$, update $T.\phi$;\\[3pt]
\phantom{\textbf{while}}\quad \textbf{if} $\text{isHumanAnchor}(B_{\text{cur}})$:\\[3pt]
\phantom{\textbf{while}}\quad\quad set $\sigma_{B_{\text{cur}}} \leftarrow \text{HUMAN\_ACKNOWLEDGED}$; append $\langle B_{\text{cur}}, \tau_{\text{now}}, \texttt{HUMAN\_RECEIVED}\rangle$; return \texttt{SUCCESS};\\[3pt]
\phantom{\textbf{while}}\quad \textbf{else}:\\[3pt]
\phantom{\textbf{while}}\quad\quad set $B_{\text{parent}} \leftarrow P_{B_{\text{cur}}}$;\\[3pt]
\phantom{\textbf{while}}\quad\quad \textbf{if} $B_{\text{parent}} = \texttt{null}$: append $\langle \texttt{null}, \tau_{\text{now}}, \texttt{NO\_ANCHOR\_FOUND}\rangle$; raise \texttt{HIERARCHY\_ERROR}; return \texttt{FAILURE};\\[3pt]
\phantom{\textbf{while}}\quad\quad set $B_{\text{cur}} \leftarrow B_{\text{parent}}$.\\[3pt]
\textbf{end while}
\end{trivlist}
...
There is no automatic resolution. Even under repeated staleness warnings, the Fact Layer will not accept a resolution from any actor other than the confirmed human anchor. If the human anchor is permanently unreachable (e.g., due to incapacitation), the escalation algorithm's hierarchy traversal provides for a \texttt{HIERARCHY\_ERROR} condition (Algorithm~\ref{alg:escalation}, the HIERARCHY\_ERROR case), which surfaces the failure to the next available human anchor in the chain of organizational authority. This fallback is the only permitted deviation from the single-anchor rule, and it is triggered only by the absence of a human anchor in the ancestry chain, not by any machine actor's preference.
