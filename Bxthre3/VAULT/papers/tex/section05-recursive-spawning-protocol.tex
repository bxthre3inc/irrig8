...
\begin{enumerate}
    ...
    \item[(V4)] \textbf{Verify scope refinement at each link.} Confirm $R(n_{j+1}) \subseteq R(n_j)$ for each consecutive pair, establishing that each scope is a descendant refinement of the human anchor's intent. This is the drift prevention condition (Equation~\ref{eq:scope-refinement}).
