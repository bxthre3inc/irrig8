...
\begin{center}
\begin{tabular}{>{\bfseries}p{2.0cm} p{3.0cm} p{3.0cm} p{2.5cm}}
\toprule
...
\begin{center}
\begin{tabular}{>{\bfseries}p{2.2cm} p{1.8cm} p{1.8cm} p{5.2cm}}
\toprule
...
\node[root] (root) {HUMAN PURPOSE LAYER \par {ootnotesize Human Root --- Accountability Anchor}};

\node[child, below left=1.2cm and 2.2cm of root] (hub1)
    {CHILD LOOP A \par {ootnotesize Worksheet OTA}};

\node[child, below right=1.2cm and 2.2cm of root] (hub2)
    {CHILD LOOP B \par {ootnotesize Worksheet OTA}};

\node[sensor, below left=1.0cm and 0.8cm of hub1] (s1) {Sensor};
\node[sensor, below right=1.0cm and 0.8cm of hub1] (s2) {Sensor};
\node[sensor, below=1.0cm of hub2] (s3) {Sensor};

\draw[arrow] (root.south west) -- (hub1.north)
    node[midway, left=3pt, label] {Spawn Worksheet};
\draw[arrow] (root.south east) -- (hub2.north)
    node[midway, right=3pt, label] {Spawn Worksheet};
...
\end{tikzpicture}
\end{center}
\caption{Recursive spawning: parent nodes deploy Worksheet containers to child nodes, each carrying immutable parent Purpose pointers. Lost connectivity triggers local execution of the last-approved Worksheet.}
