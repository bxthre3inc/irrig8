...
The liveness guarantee depends on one condition: that the spawning hierarchy is structurally sound. A broken parent pointer — which could arise from a bug in the spawning implementation or deliberate corruption of the hierarchy — would represent a HIERARCHY\_ERROR condition, which Algorithm~\ref{alg:escalation} detects and logs explicitly. The error is not silently absorbed. The Forensic Ledger records the HIERARCHY\_ERROR, and the system surfaces the failure to the next available human anchor in the organizational chain. The liveness guarantee is therefore conditional on hierarchy integrity, which is itself enforced by the Fact Layer at spawn time through the parent pointer immutability constraint.
...
\noindent\textit{Proof Sketch.} The proof traces the lifecycle of a BailoutTrigger through the Forensic Ledger and shows that the principal field is both mandatory and authenticated.
...
\noindent\textit{Step 5 — Chain integrity.} Every entry in the above sequence is SHA-256 chain-sealed (Equation~\ref{eq:hash-chain}) at the time of commitment. The \texttt{principal} field is part of the sealed content of the RESOLVED entry. Modifying this field after commitment would change the entry's content hash, breaking the chain at that entry and all subsequent entries. The tamper-evidence property of the Forensic Ledger (Section~\ref{sec:tamper-evidence}) ensures that any such modification is detectable in O($n$) verification time, requiring no access to internal system state. $\square$

\bigskip

The Accountability guarantee is structurally dependent on the immutability of the $h(n)$ anchor reference and the Fact Layer's exclusive write authority over the Forensic Ledger. If a machine actor could modify $h(n)$ after node creation, it could substitute a different human identity at resolution time. If the Fact Layer did not have exclusive write authority over the ledger, a compromised Bounds Engine could fabricate a RESOLVED entry with a fabricated principal. Both of these attacks are architecturally prevented by the BX3 layer separation: $h(n)$ is immutable by the Purpose Layer's structural constraint on anchor references, and the Forensic Ledger is append-only with Fact Layer exclusive write access.
...
The three guarantees are not independent — they interact in ways that strengthen the overall accountability posture. This section briefly notes the key interaction effects.
...
The three guarantees are independently verifiable by an external auditor without access to internal system state or cryptographic keys. Verification proceeds as follows:

\medskip

\noindent\textbf{Safety verification.} Retrieve the Forensic Ledger entries for all BailoutTrigger events in a given operational window. For each trigger, confirm that: (a) no ActuatorCommand entry appears in the ledger between the BOUNDED entry and the RESOLVED entry for the same trigger ID; and (b) the RESOLVED entry carries a Purpose Layer credential with a non-null \texttt{principal} field. If condition (a) holds for all triggers, the Safety invariant (Invariant~\ref{inv:safety}) has been upheld for the window. If condition (b) holds for all triggers, the Accountability invariant (Invariant~\ref{inv:principal-attribution}) has been upheld.

\medskip

\noindent\textbf{Liveness verification.} For each trigger in the window, retrieve the propagation chain $\phi$ from the RESOLVED entry. Verify that the chain terminates at a Purpose Layer anchor (i.e., the final entry in $\phi$ has $\texttt{HUMAN\_RECEIVED}$). Verify that the chain depth $d$ is finite and consistent with the known hierarchy depth of the system. Identify any trigger that terminates with HIERARCHY\_ERROR and confirm that the error was surfaced to an operational administrator.

\medskip

\noindent\textbf{Accountability verification.} For each RESOLVED entry, retrieve the \texttt{principal} field and verify: (a) the principal is a member of the registered Purpose Layer principal set $\mathcal{P}$ (this requires access to the principal registry, which is maintained by the Purpose Layer and is outside the scope of the Forensic Ledger); and (b) the SHA-256 chain integrity holds from the previous entry through this entry. Perform a full chain verification on the ledger as a whole using the O($n$) forward-pass algorithm specified in Section~\ref{sec:tamper-evidence}.
