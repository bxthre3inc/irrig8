...
The Bailout Protocol is not a safety module added to an existing autonomous system. It is the runtime expression of the BX3 Framework's upstream accountability guarantee: that every exception state at any Bounds Engine node propagates to a human accountability anchor, bypassing all machine intermediaries. Making this guarantee architecturally enforceable rather than merely policy-level requires that the Bailout Protocol be integrated into all three functional layers at their boundaries. This section specifies that integration: how the \purpose\ anchors human accountability at the top, how the \bounds\ detects and fires bailout signals, how the \fact\ enforces the hard block and maintains the forensic record, and how the state machine operates as a deterministic extension of the \fact.

\subsection{The Purpose Layer as Human Accountability Anchor}
\label{sec:purpose-anchor}

The \purpose\ is the only layer that may legitimately terminate a Bailout Protocol signal. This is a definitional commitment, not a design choice among alternatives. The \purpose\ is defined by its capacity for genuine accountability --- the ability to bear responsibility for a decision in a way that is legally, socially, and organizationally enforceable. A machine actor, regardless of its sophistication, cannot bear responsibility in this sense. It can optimize, it can recommend, and it can defer --- but it cannot be held accountable in the way a named human can. Therefore, only a \purpose\ actor may receive and resolve a Bailout signal.

In the BX3 three-layer model, every node in a recursive system carries a \textit{Purpose} authored and owned by a named human at the \purpose. This Purpose is a binding specification: it defines what the node exists to accomplish, what constraints govern its operation, and who is accountable for its outcomes. It is not a preference or a suggestion. When a node's \bounds\ encounters a condition it cannot resolve within those constraints, the bailout signal does not route to an intermediate machine tier for a second autonomous attempt. It routes directly to the named \purpose\ owner of that node, with complete diagnostic context.

The \purpose\ serves a dual function in the Bailout Protocol:

\begin{enumerate}
\item \textbf{Authorization provenance.} The \purpose\ establishes the authoritative chain from which all legitimate system action derives. Every \bounds\ action traces back to a \purpose\ authorization. If a bailout fires, the \purpose\ owner receives the complete chain of prior authorizations and proposed actions, enabling informed judgment about how to proceed rather than requiring them to reconstruct context from fragments.
...
\subsection{The Bounds Engine Triggering Bail-Out}
\label{sec:bounds-trigger}

The \bounds\ is the layer that detects and fires bailout signals. This is a direct consequence of its defining role: the \bounds\ evaluates proposed actions against encoded constraints and either approves, rejects, or escalates. When the situation exceeds its resolution authority --- a novel condition outside existing constraints, a conflict between competing imperatives, or a proposed action that would violate a hard \fact\ enforcement rule --- the \bounds\ does not guess, does not default to a safest-known action, and does not recursively attempt autonomous resolution. It fires the Bailout Protocol.
...
The \fact\ enforces a \textit{hard block} on any physical actuator command that would affect the node associated with a live bailout signal. When a \texttt{BailoutTrigger} is in the \texttt{fired} or \texttt{escalating} state, the \fact\ refuses to forward any \bounds\ command to physical actuators associated with the affected node. This is not a software conditional:
...
The block cannot be overridden by a higher-confidence \bounds\ proposal, a changed business context, a timeout, or any machine actor. The block holds until the \texttt{BailoutTrigger} reaches the \texttt{human\_review} state and the \purpose\ owner issues an explicit resolution. This is architecturally indistinguishable from the way the \fact\ enforces any other deterministic constraint: the block is a property of the \fact's state machine, not a conditional check that can be bypassed.

The Forensic Ledger maintained by the \fact\ records every bailout event in real time across three planes of the nine-plane structure (Plane 4: Accountability, Plane 6: Temporal Sequencing, Plane 8: Compliance Posture). A dedicated \texttt{BailoutEventRecord} schema extends the standard event record:
...
The append-only, hash-chained structure ensures that any modification to a prior entry --- whether accidental or deliberate --- breaks the hash chain at the point of modification and is detectable by any verifier performing a chain integrity check. The \fact\ performs this check automatically after every event write and triggers a bailout if a break is detected, ensuring that ledger integrity is itself subject to the upstream accountability guarantee.
...
The upstream accountability guarantee --- that systems built on the BX3 Framework \textit{fail upward into human consciousness and never downward into algorithmic chaos} --- is a structural property of the framework, guaranteed by the layer architecture. The Bailout Protocol is the runtime mechanism by which this guarantee is effectuated in real-time execution.
...
\subsection{Forensic Ledger Recording of Bailout Events}
\label{sec:forensic-recording}

Every bailout event generates a corresponding entry in the Forensic Ledger maintained by the \fact. The entry is designed to support three distinct use cases: real-time diagnostic review by the \purpose\ owner receiving the bailout, post-hoc audit by regulatory or legal reviewers, and automated integrity verification by monitoring systems.

The \texttt{BailoutEventRecord} schema is defined as an extension to the standard nine-plane event record, with specific fields in Planes 4 (Accountability) and 8 (Compliance Posture) capturing the bailout-specific data:

\begin{equation}
\text{BailoutEventRecord} = \langle\ \texttt{bailoutId},\ \texttt{triggerCondition},\ \texttt{originatingNodeId},\ \texttt{propagationChain},\ \texttt{diagnosticSnapshot},\ \texttt{purposePlaneRecord},\ \texttt{factPlaneRecord},\ \texttt{resolution},\ \texttt{chainIntegrityHash}\ \rangle
\end{equation}

Each entry is SHA-256 sealed and linked to the preceding entry's hash, forming the forward-linked chain required for tamper evidence. The \fact\ verifies chain integrity after every write. A break in the chain triggers an immediate bailout of the verification process itself --- ensuring that ledger integrity is subject to the same upstream accountability guarantee as all other system events.

The propagation chain within each entry solves the abstraction leakage problem. When a human operator is inserted into a running system to resolve a bailout, they frequently lack the contextual information needed to understand the node's position in the broader system hierarchy. The propagation chain in the Forensic Ledger entry preserves the complete hierarchical path from the triggering node to the human anchor, with timestamps and actions at each hop, ensuring the \purpose\ owner receiving the bailout has full context for decision-making.

In the context of Irrig8, the Bailout Protocol's forensic recording supports the water usage audit trail required by Colorado Water Court reporting. Each bailout event records not only the decision context but also the complete constraint state at the time of firing, providing regulators with evidence that autonomous irrigation decisions were under continuous human accountability control and that no irrigation action was taken during an unresolved exception state without human authorization.
