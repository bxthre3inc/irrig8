...
Each transition in Table~\ref{tab:bailout-transition} is governed by a precondition drawn from the node's operational context. We define each transition formally as a guarded command.
...
\item[\textbf{T4: BAIL\_PENDING $\rightarrow$ ESCALATING (protocol activation).}] Transition T4 fires immediately upon entry to BAIL\_PENDING — there is no dwell time, no retry attempt, and no machine-actor intervention permitted. Formally:
\begin{equation}
\text{T4 fires} \iff \text{state} = \text{BAIL\_PENDING}. \tag{T4}
\end{equation}
This transition is atomic and is enforced directly by the Fact Layer pre-commit hook, bypassing any agent-level code. It is the architectural guarantee that bailout initiation cannot be suppressed by any machine actor, including the Bounds Engine that originally requested the timeout.

\item[\textbf{T5: ESCALATING $\rightarrow$ HUMAN\_ACKNOWLEDGED (human receipt).}] Transition T5 fires when the human anchor $h(n)$ acknowledges receipt of the exception. Acknowledgment does not constitute a binding directive — it confirms only that the human has observed the exception and is actively reviewing it. Formally:
\begin{equation}
\text{T5 fires} \iff \text{state} = \text{ESCALATING} \land \text{acknowledged}(h(n), \text{ctx}). \tag{T5}
\end{equation}
The \texttt{acknowledged} predicate is satisfied when $h(n)$ issues an explicit \texttt{ACK} signal through the Root Dashboard or an equivalent authorized interface. No machine actor may issue this signal.

\item[\textbf{T6: ESCALATING $\rightarrow$ ESCALATING (propagation retry).}] Transition T6 fires when the propagation timeout $T_{\text{prop}}$ expires without receiving an acknowledgment from the parent node. The algorithm retries with exponential backoff. Formally:
\begin{equation}
...
\Procedure{Escalate}{$n$, trigger}
  ...
  \Repeat
    ...
    \State \Call{UpdateLedger}{\text{retry}, $n$, $p$, $\text{attempts}$, $T_{\text{prop}}$}
  \Until $\text{attempts} \geq N_{\max}$
  \State \Call{FireTimeoutEvent}{\text{parent\_unreachable}, $n$, $p$, ctx}
  \State \Return \texttt{timeout}
\EndProcedure
\Statex
\Procedure{ForwardException}{$n$, ctx}
  \State $\text{ctx} \leftarrow \text{ctx} \;\cup\; \{\text{node\_id}: n,\ \text{timestamp}: \text{clock},\ \text{state\_snapshot}: \text{getstate}(n)\}\$
  \State \Call{UpdateLedger}{\text{forward}, $n$, $\text{ctx}$}
  \State $p \leftarrow \alpha(n)$
...
\Procedure{BuildDiagnosticContext}{$n$, trigger}
  ...
  \State $\text{ctx.exception} \leftarrow \text{trigger}$
  \State $\text{ctx.originating\_node} \leftarrow n$
  \State $\text{ctx.human\_root} \leftarrow h(n)$
  \State $\text{ctx.parent\_chain\_depth} \leftarrow \text{distance}(n, h(n))$
  \State $\text{ctx.node\_state\_snapshot} \leftarrow \text{getstate}(n)$
  ...
  \State $\text{ctx.bounds\_explanation} \leftarrow \text{BoundsEngine.explain}(\text{trigger})$
  \State $\text{ctx.resolution\_history} \leftarrow \text{getresolutionhistory}(n, \text{trigger})$
  \State \Return ctx
\EndProcedure
\Statex
\Procedure{SelectPathway}{PHR, $p$}
  \State $\text{candidates} \leftarrow \{w \in \text{PHR.pathways} \mid p \in w.\text{reachable\_nodes} \land w.\text{status} \neq \text{DEGRADED}\}$
  \State \Return $\arg\min_{w \in \text{candidates}} w.\text{estimated\_latency}$
\EndProcedure
\end{algorithmic}
\end{algorithm}

\subparagraph{Propagation chain integrity.} The propagation chain $\text{ctx.propagation\_chain}$ is append-only: no actor in the propagation path may modify, truncate, redact, or suppress any field of $\text{ctx}$. Each node appends a record to $\text{ctx.propagation\_chain}$ containing: its node identifier, a timestamp, a state snapshot, and any locally relevant diagnostic metadata. This append-only integrity property is enforced by the Fact Layer: the \texttt{UpdateLedger} call is atomic, and any attempt to modify an existing ledger entry is rejected and logged as a protocol violation.
