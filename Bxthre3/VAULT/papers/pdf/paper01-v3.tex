...
\draw[arrow, detgray!80, bend left=65] (det.east) to
    node[midway, right=4pt, label] {violations blk} (ai.east);

\end{tikzpicture}
\end{center}

\caption{The BX3 Framework: Three immutable functional layers. The Fact Layer hard-blocks Bounds Engine violations. The forensic ledger and escalation path closes the accountability loop.}
\label{fig:bxb-stack}
\end{figure}
\vspace{1em}
...
\begin{center}
\begin{tabular}{>{\bfseries}p{2.5cm} p{2.0cm} p{2.0cm} p{5.5cm}}
\toprule
\textbf{Interface} & \textbf{From} & \textbf{To} & \textbf{What Crosses the Boundary} \\
\midrule
Direction & Purpose Layer & Bounds Engine & Goals, SLOs, constraints \\
\addlinespace
Proposal & Bounds Engine & Fact Layer & Action proposals, simulation outputs \\
\addlinespace
Hard Block & Fact Layer & Bounds Engine & Violation signals, blocked receipts \\
\addlinespace
Forensic Feedback & Fact Layer & Purpose Layer & Ledger events, escalations \\
\addlinespace
Override & Purpose Layer & Fact Layer & Direct commands, emergency halt \\
\addlinespace
Escalation & Any node & Purpose Layer & Bailout signals \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[scale=0.58,every node/.style={transform shape},node distance=1.4cm and 0.6cm,
pillar/.style={rectangle,rounded corners=5pt,fill=warnorange!10,draw=warnorange!60,minimum width=4.8cm,minimum height=1.7cm,font=\footnotesize,inner sep=4pt}]
\node[pillar] (p1) at (-3.0cm,0) {PILLAR 1\emph{Loop Isolation}\Prevents logic collisions by architectural separation};
\node[pillar] (p2) at (3.0cm,0) {PILLAR 2\emph{Recursive Spawning}\Distributed local autonomy via Worksheets};
\node[pillar] (p3) at (-3.0cm,-2.7cm) {PILLAR 3\emph{Sandbox Gate}\Validates proposals in digital twin};
\node[pillar] (p4) at (3.0cm,-2.7cm) {PILLAR 4\emph{Spatial Firewall}\Isolates data planes by architecture};
\node[pillar,fill=warnorange!25,draw=warnorange,minimum width=5.8cm] (p5) at (0cm,-5.4cm) {PILLAR 5\emph{Bailout Protocol}\Upstream accountability on all exceptions};
\draw[-{Stealth[length=5pt]},thick,humanblue](p1)--(p2) node[midway,above,font=\footnotesize]{shared Purpose};
\draw[-{Stealth[length=5pt]},thick,humanblue](p1)--(p3) node[midway,left,font=\footnotesize]{enforces};
\draw[-{Stealth[length=5pt]},thick,humanblue](p2)--(p4) node[midway,right,font=\footnotesize]{enforces};
\draw[dashed,-{Stealth[length=5pt]},thick,gray](p3.south west)..controls++(-0.4cm,-0.5cm)and++(-0.4cm,-0.5cm)..(p5.north west);
\draw[dashed,-{Stealth[length=5pt]},thick,gray](p4.south east)..controls++(0.4cm,-0.5cm)and++(0.4cm,-0.5cm)..(p5.north east);
\node[below=0.2cm of p5,font=\footnotesize\itshape,text=gray]{All five pillars enforce the three layer postulates};
\end{tikzpicture}
}
\centering\caption{\caption{The Five Pillars: structural mechanisms that enforce the three layer postulates. Each pillar addresses a specific failure mode that emerges when AI, deterministic, and human systems are combined without architectural discipline.}
\end{figure}
\label{fig:pillars}
\end{figure}
