...
\centering
\textbf{\large Layer 1: PURPOSE LAYER} --- Intent | Judgment | Accountability \\
\textit{Human Root Mandate} \\
\textbf{Layer 2: BOUNDS ENGINE} --- Analysis | Bounded Reasoning | Proposal \\
\textit{Can propose, cannot execute unauthenticated} \\
\textbf{Layer 3: FACT LAYER} --- Firewall | Deterministic | Hard-blocks \\
\textit{100\% deterministic} \\
\vspace{0.5em}
\begin{tabular}{p{0.45\textwidth}p{0.45\textwidth}}
\textbf{Down $\downarrow$:} Goals, SLOs, Constraints & \textbf{Up $\uparrow$:} Ledger, Escalation \\
\textbf{Down $\downarrow$:} Authorization scope & \textbf{Up $\uparrow$:} Hard blocks, Violations
\end{tabular}
\end{minipage}
\caption{The BX3 Framework: Three immutable functional layers. The Fact Layer hard-blocks Bounds Engine violations; the forensic ledger closes the accountability loop.}
\label{fig:bxb-stack}
\end{figure}
\vspace{1em}
...
\begin{tabular}{>{\bfseries}p{3cm} p{2.2cm} p{2.2cm} p{5.1cm}}
\toprule
...
\begin{minipage}{0.95\textwidth}
\centering
\textbf{HUMAN PURPOSE LAYER} --- Human Root \\
\textbf{Child Loop A} (Worksheet OTA $\to$ Local Fact Layer) \\
\textbf{Child Loop B} (Worksheet OTA $\to$ Local Fact Layer) \\
Sensor Node \quad Sensor Node \quad Sensor Node \\
\vspace{0.5em}
\textit{Each Worksheet carries an immutable parent Purpose hash. Children execute their last-known-good Worksheet in degraded network conditions.}
\end{minipage}
\caption{Recursive spawning and the audit trail. Each spawned node inherits a Worksheet, ensuring the audit trail cannot be broken by context loss.}
