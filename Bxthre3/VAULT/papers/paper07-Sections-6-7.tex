...
\begin{itemize}
    \itemsep0em
    \item \textbf{Accountability trace:} Given any node $c$, reconstruct $\chi_{\mathcal{L}}(c)$ by following spawn entries backward. This returns the authorized spawning chain, which must match the in-memory pointer chain $\chi(c)$ by Theorem~\ref{thrm:chain-integrity}.
    \item \textbf{Drift detection:} Given a node $c$ and its observed behavior $b$, verify whether $b \models \sigma_c$ for the authorized scope $\sigma_c$ in $c$'s spawn entry. A mismatch constitutes drift and triggers the Bailout Protocol.
    \item \textbf{Depth verification:} Given any node $c$, recompute $d(c)$ from spawn entries in $\mathcal{L}$ and verify $d(c) \leq d_{\max}$. A violation indicates either unauthorized depth escalation or retroactive ledger tampering.
\end{itemize}
...
\textit{Case 1: $c$ attempts to modify $\alpha(c)$.} By hardware-enforced immutability, $\alpha(c)$ is stored in a write-once, MPU-protected field at the Fact Layer. Any modification requires Fact Layer authorization, which requires a Purpose Layer signature in the forensic ledger. The modification is detectable at the moment of the attempt, and the Bailout Protocol is triggered. The drift is not undetectable.

\textit{Case 2: $c$ acts on $p'$ without modifying $\alpha(c)$.} Any action taken on an unauthorized purpose produces observable state in the Fact Layer — sensor inputs, actuator commands, ledger entries. The forensic ledger records all such states. Divergence is detectable by comparing observed behavior against the authorized scope $\sigma_c$ recorded at spawn. Since the divergence is detectable, condition (a) is violated.

In both cases, drift is detectable. By the definition of the Bailout Protocol, any detectable drift triggers mandatory escalation to a Purpose Layer actor. Escalation traverses the parent pointer chain upward, which is finite by the Termination Theorem (Theorem~\ref{thrm:termination}), so the drift is rectifiable. Both conditions (a) and (b) cannot hold simultaneously; drift is therefore structurally prevented. $\square$
\endproof

The key structural insight is that all three components are jointly necessary: immutable parent pointers prevent unauthorized chain manipulation, the forensic ledger makes behavioral divergence observable, and termination guarantees that the accountability chain through which divergence is detected is always finite.
...
This corollary enables multi-domain deployments — such as Irrig8, which requires separate spawning hierarchies for irrigation scheduling, equipment health monitoring, and regulatory reporting — where each domain operates as an independent accountability tree under a common Purpose Layer actor, with cross-domain coordination occurring exclusively through Purpose Layer authorization rather than ad hoc inter-node communication.
