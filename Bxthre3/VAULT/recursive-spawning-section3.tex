...
\end{enumerate}
Orphaning severs the delegation chain at the parent node. The agent $n$ persists in operational state but is structurally disconnected from its chain of authorization. Connectivity and authorization are distinct properties: a living parent pointer does not guarantee a valid authorization chain.

\medskip
\noindent\textbf{Definition 2 (Drifted Agent).}
An agent $n$ is \emph{drifted} if the chain $C(n) = \langle n_0, n_1, \ldots, n \rangle$ obtained by repeatedly following $\alpha$ does not terminate at a human principal. Equivalently:
\begin{equation}
\mathrm{drifted}(n) \;\iff\; \neg\,\exists\, h \in \mathcal{H} : \alpha^{\mathrm{depth}(n)}(n) = h.
\label{eq:rs-drifted-def}
\end{equation}
A drifted agent may be orphaned (parent terminated) or may have a living parent whose own chain does not reach any human principal. Drift is a property of the chain's origin, not of the chain's intermediate connectivity. An agent whose parent is alive and responsive but whose delegation chain terminates at a non-human node is drifted by this definition.
...
\end{equation}
An agent enters the drift triad when all three conditions hold simultaneously: it is structurally disconnected from its parent, its delegation chain does not reach any human anchor, and it continues to execute its event loop. Any two of the three conditions in isolation do not constitute the drift triad. An orphaned agent whose chain reaches a human principal is an operational anomaly but not a drift failure; a drifted agent with a living parent is a chain topology defect but not an active operational threat; an operating agent whose chain reaches a human principal is functioning normally. The triad is the conjunction.

The four possible combinations of the binary sub-modes (orphaned, drifted) and operating state are given in Table~\ref{tab:rs-drift-submodes}.
...
The deeper structural reason depth limits are insufficient is that they address the wrong dimension of the accountability problem. Depth limits constrain chain length — a structural property of the spawn graph. Drift, however, is fundamentally about authorization scope: whether each node's operating envelope is a legitimate descendant refinement of the human anchor's intent. A drifted agent at depth $1$ with unbounded scope expansion authority can produce consequences that exceed the human anchor's intent as severely as a drifted agent at depth $D_{\max} - 1$. Length and authority are orthogonal properties.

% -------------------------------------------------------------------------
\subsection{Formal Model}
\label{sec:rs-drift-model}

We present a compact formal model capturing the structural elements essential to drift analysis. This model is referenced in Sections~\ref{sec:rs-protocol} through~\ref{sec:rs-correctness} and used in the RSP correctness proofs.
...
\begin{equation}
\mathrm{chain-reaches}(n, h) \;\iff\; \alpha^{\mathrm{depth}(n)}(n) = h.
...
\begin{equation}
\bigl( \alpha(n) = \bot \;\lor\; \mathrm{unreachable}(\alpha(n)) \bigr)
\;\land\;
\bigl( \neg\,\exists\, h \in \mathcal{H} : \mathrm{chain-reaches}(n, h) \bigr)
\;\land\;
\mathrm{operating}(n) = \mathrm{true}.
...
\begin{equation}
\mathrm{Remediate}(a):\quad \alpha(c) := h \text{ for some } h \in \mathcal{H}.
