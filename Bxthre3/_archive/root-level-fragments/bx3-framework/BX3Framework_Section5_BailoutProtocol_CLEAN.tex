...
\begin{align}
\text{INV}_3 &: \text{state}(n) = \text{RESOLVED} \implies \exists d \in \{\text{ACK}, \text{RESOLVE}, \text{REJECT}\} \land \text{directive}(n) = d. \tag{INV-3}
\end{align}

That is, every RESOLVED state is terminal with a recorded directive from $h(n)$. There are no unresolved RESOLVED states.

% --------------------------------------------------------------
\subsection{Transition Conditions}\label{sec:transitions}
% --------------------------------------------------------------

Each transition T1–T9 is associated with a formal precondition expressed as a predicate on the node's current state, the Fact Layer worksheet, and the incoming event. The Bailout Monitor evaluates all transition preconditions on every event and commits at most one transition per event to the Forensic Ledger.

\bigskip
\noindent\textbf{Transition T1 (Trigger).} $\text{IDLE} \rightarrow \text{BOUNDED}$

Precondition: A trigger event $e_{\text{trigger}}$ arrives at node $n$ carrying an exception $x$ that satisfies the primary trigger condition TC or the secondary trigger condition (explicit \texttt{bailout\_signal}):
...
\bigskip
\noindent\textbf{Transition T2 (Authorization from Bounds).} $\text{BOUNDED} \rightarrow \text{HUMAN\_ACKNOWLEDGED}$

Precondition: The \fact{} layer receives an authorization confirmation $e_{\text{auth}}$ from the \bounds{} layer within $T_{\text{bounds}}$:

\begin{equation}
\text{T2 fires} \iff \text{state} = \text{BOUNDED} \land e_{\text{auth}} \text{ received before } T_{\text{bounds}}. \tag{T2}
\end{equation}

\bigskip
\noindent\textbf{Transition T3 (Bail Pending).} $\text{BOUNDED} \rightarrow \text{BAIL\_PENDING}$

Precondition: A timeout signal $e_{\text{timeout}}$ fires for $T_{\text{bounds}}$ without a \fact-layer-approved resolution:

\begin{equation}
\text{T3 fires} \iff \text{state} = \text{BOUNDED} \land T_{\text{bounds}} \text{ expired} \land \neg\text{resolution\_approved}. \tag{T3}
\end{equation}

\bigskip
\noindent\textbf{Transition T4 (Escalate).} $\text{BAIL\_PENDING} \rightarrow \text{ESCALATING}$

Precondition: The node is in BAIL\_PENDING and receives the compounded timeout signal $(T_{\text{bounds}}, T_{\text{prop}})$:

\begin{equation}
\text{T4 fires} \iff \text{state} = \text{BAIL\_PENDING} \land e_{\text{timeout}} = (T_{\text{bounds}}, T_{\text{prop}}). \tag{T4}
\end{equation}

Entry to ESCALATING is atomic: the \fact{} layer immediately invokes \texttt{SendToParent} without an intervening machine-actor decision step.

\bigskip
\noindent\textbf{Transition T5 (Human Ack during Escalation).} $\text{ESCALATING} \rightarrow \text{IDLE}$

Precondition: The human anchor receives the exception and issues an acknowledgment \texttt{ACK} that requires no further action:
...
\bigskip
\noindent\textbf{Transition T6 (Timeout during Escalation).} $\text{ESCALATING} \rightarrow \text{ESCALATING}$

Precondition: A propagation timeout $T_{\text{prop}}$ fires during escalation but retry budget remains:

\begin{equation}
\text{T6 fires} \iff \text{state} = \text{ESCALATING} \land e_{\text{timeout}} = T_{\text{prop}} \land \text{retry\_count} < N_{\max}. \tag{T6}
\end{equation}

The algorithm retries via \texttt{SelectPathway} and \texttt{SendToParent} with exponential backoff applied to $T_{\text{prop}}$.

\bigskip
\noindent\textbf{Transition T7 (Binding Directive from Escalating).} $\text{ESCALATING} \rightarrow \text{RESOLVED}$
...
\item[\textbf{Input:}] node $n$, exception $e$, diagnostic context $C$
\item[\textbf{Output:}] terminal state reached, directive issued by $h(n)$
\item[]
\item $pathway \leftarrow \texttt{SelectPathway}(n)$
\item $retry\_count \leftarrow 0$; $T_{\text{current}} \leftarrow T_{\text{prop}}$
\item[]
...
\item[]\hspace{1em}\textbf{if} $ack\_received = \text{true}$ \textbf{then}
\item[]\hspace{2em}$state \leftarrow \text{HUMAN\_ACKNOWLEDGED}$; \textbf{goto} \texttt{WaitForDirective}
...
\item[]\hspace{1em}$pathway \leftarrow \texttt{SelectPathway}(n)$
\item \textbf{end while}
\item[]
\item All pathways exhausted at $T_{\text{cap}}$
\item $state \leftarrow \text{ESCALATING}$; $directive \leftarrow \texttt{PATHWAY\_EXHAUSTED}$
\item \textbf{return} $(\text{RESOLVED},\; \texttt{PATHWAY\_EXHAUSTED})$
\item[]
\item \texttt{WaitForDirective:}
\item $directive \leftarrow \texttt{ReceiveDirective}(h(n), T_{\text{human}})$
\item \textbf{if} $directive = \texttt{timeout}$ \textbf{then}
\item[]\hspace{1em}\textbf{record} $\texttt{unresolved\_tag}$ in Forensic Ledger
\item[]\hspace{1em}\textbf{return} $(\text{RESOLVED},\; \texttt{unresolved})$
\item \textbf{end if}
\item $directive\_type \leftarrow \texttt{Parse}(directive)$
\item \textbf{if} $directive\_type = \texttt{ACK}$ \textbf{then} \textbf{return} $(\text{IDLE},\; \texttt{ACK})$
\item \textbf{else if} $directive\_type \in \{\texttt{RESOLVE},\; \texttt{REJECT}\}$ \textbf{then} \textbf{return} $(\text{RESOLVED},\; directive\_type)$
\item \textbf{end if}
\end{trivlist}
...
\noindent\textbf{\texttt{SelectPathway} function.}
\begin{trivlist}\itemsep=0pt
\item[\textbf{Input:}] node $n$
\item[\textbf{Output:}] selected communication pathway $p$
\item[]
\item $P \leftarrow \text{PathwayHealthRegistry.get\_functional\_pathways}(h(n))$
\item \textbf{if} $P = \emptyset$ \textbf{then} \textbf{return} $\text{null}$ (no functional pathway)
\item $p \leftarrow \arg\min_{p' \in P} \text{estimated\_latency}(p')$
\item \textbf{return} $p$
\end{trivlist}
...
\noindent\textbf{\texttt{SendToParent} function.}
\begin{trivlist}\itemsep=0pt
\item[\textbf{Input:}] node $n$, exception $e$, context $C$, pathway $p$, timeout $T$
\item[\textbf{Output:}] boolean acknowledgment
\item[]
\item $payload \leftarrow \texttt{BuildEscalationPayload}(n, e, C)$
\item $sent\_at \leftarrow \text{current\_timestamp}$
\item $A \leftarrow \text{Channel}(p).\texttt{send}(payload)$
\item \textbf{wait for} $A$ \textbf{or} $T$:
\item[]\hspace{1em}\textbf{if} $\text{received}(A)$ \textbf{then return} $\text{true}$
\item \textbf{end wait}
\item \textbf{return} $\text{false}$
\end{trivlist}
...
\begin{trivlist}\itemsep=0pt
\item[\textbf{Definition (Bypass Property)}\label{def:bypass}]
