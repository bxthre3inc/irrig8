...
Before specifying the Recursive Spawning Protocol, we must precisely characterize the failure mode it targets. \emph{Autonomous drift} — the condition in which an agent continues to operate with no traceable authorization path to a human principal — is not a configuration error. It is a structural outcome produced by specific architectural conditions in conventional multi-agent runtimes. This section provides the formal vocabulary for that characterization, defines the constituent failure sub-modes, identifies the three enabling structural conditions, catalogs four concrete failure modes, and proves formally that depth limits alone are insufficient to prevent drift.
