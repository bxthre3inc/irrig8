...
The Recursive Spawning Protocol provides structural guarantees that no conventional architecture achieves: immutable delegation chains enforced at runtime, scope refinement as a structural invariant rather than a policy convention, and compulsory escalation to a human principal at a Purpose Layer-defined depth. These guarantees are not free. RSP imposes concrete costs that implementers must weigh against their operational requirements, and each cost defines a direction for future work. This section examines three principal limitations.

% ------------------------------------------------------------
\subsection{Performance Overhead}
\label{sec:rs-perf-overhead}

Every spawn event under RSP requires sequential validation across three layers that conventional \texttt{fork()} does not perform: the Purpose Layer authorizes the spawn warrant, the Bounds Engine evaluates depth against $D_{\max}$, and the Fact Layer executes the pre-commit hook and generates the hash-chained ledger entry. In the AgentOS runtime, this sequence adds \numrange{15}{40} milliseconds per spawn under nominal load; cryptographic signing and hash-chaining in the Fact Layer account for approximately 60\% of this cost. A single \texttt{fork()} in a conventional runtime completes in microseconds.

This overhead is the structural price of the immutable parent pointer and the tamper-evident forensic ledger. It is not a defect. High-frequency spawn patterns — per-document children in batch workloads, per-step children in generative agentic loops — accumulate this latency and degrade end-to-end throughput relative to naive forking. The prescribed mitigation is batched spawning: a parent provisions fewer children, each handling multiple work units, trading coarser decomposition granularity for lower cumulative spawn overhead.

The forensic ledger's append-only Merkle-tree structure imposes proportional storage and I/O overhead. Under sustained high-concurrency load — thousands of concurrent sessions each producing spawn events — the ledger write path can become a throughput bottleneck if the storage layer is not provisioned for write-intensive append-only workloads. Write-ahead log buffering and periodic Merkle-tree checkpoint compaction are the current runtime mitigations. Ledger growth management in long-running high-throughput deployments remains an open engineering problem.

The overhead is architecturally necessary: the pre-commit hook and the immutable parent pointer are what make RSP's guarantees structural rather than advisory. A system that bypasses these checks to improve throughput is not a faster implementation of RSP — it is a different, weaker protocol that lacks RSP's correctness properties.

% ------------------------------------------------------------
\subsection{Scalability of the Spawn Chain Architecture}
...
The inability to dynamically adjust depth is a deliberate design property: relaxing it permits unbounded chain growth and collapses the termination guarantee. The current mitigation — routing depth-bound escalations to the human anchor — is correct but introduces latency that is unacceptable for real-time workloads. A productive direction is \emph{hierarchical depth budgeting}, in which the Purpose Layer pre-authorizes a depth budget for a subtree at provisioning time, enabling bounded adaptive growth within the subtree without per-spawn human involvement. The budget ceiling remains non-negotiable: a machine actor that could extend its own budget would be equivalent to removing the bound entirely and would constitute a protocol violation.
...
A third constraint is the human anchor as single escalation terminus. Simultaneous escalations from multiple chains converge on the same human principal, who becomes a sequential bottleneck. If ten concurrent chains reach $D_{\mathrm{ESCALATE}}$ simultaneously, the system must queue or permit parallel \texttt{ACK}/\texttt{RESOLVE}/\texttt{REJECT} processing, both of which introduce directive attribution complexity. A Purpose Layer proxy that issues \texttt{ACK} directives for pre-authorized escalation categories — reducing per-escalation human involvement while preserving human accountability — is the intended direction. Structured directive templates are a prerequisite for this feature.

% ------------------------------------------------------------
\subsection{Compromised Purpose Layer}
\label{sec:rs-purpose-compromise}

The most structurally significant limitation is the compromised Purpose Layer problem: RSP's guarantees are contingent on the human anchor's continued availability, competence, and good faith. If unavailable, the chain stalls at $D_{\mathrm{ESCALATE}}$. If the human issues a directive without adequate diagnostic context — due to cognitive load, miscommunication, or adversarial manipulation — the protocol has no mechanism to contest or qualify it. If the human anchor itself is compromised, the entire chain above that anchor is subverted retroactively.
...
Operational success under Purpose Layer compromise depends on mitigations partially outside the protocol's scope. Availability mitigations — multi-signer Purpose Layer configurations requiring quorum approval for root provisioning — are architecturally feasible but introduce coordination complexity. Competence mitigations — structured directive templates requiring the human anchor to confirm specific diagnostic fields before issuing \texttt{RESOLVE} or \texttt{REJECT} — are within scope and should be developed in future iterations.
...
\textbf{Bounded dissent annotation.} A Fact Layer mechanism enabling nodes to record a dissent annotation when a directive is believed inconsistent with recorded intent, without override capability. Creates a permanent forensic trail for post-hoc review. Must not enable directive refusal.
...
The protocol's three correctness properties establish the operational meaning of the immutable parent pointer. Drift prevention (Theorem 1) guarantees that every node in a spawn chain operates within a scope that is a descendant refinement of the root human's authorized intent — the chain cannot migrate purpose away from its human anchor through iterative scope expansion. Chain integrity (Theorem 2) guarantees that chain topology and node configurations are tamper-evident after provisioning — no actor, including a compromised node, can alter parentage or configuration to obscure accountability. Termination (Theorem 3) guarantees that the chain cannot grow indefinitely — the depth bound is enforced by the Fact Layer as a structural invariant, not applied as a policy convention. The forensic ledger provides the evidentiary substrate that makes all three properties auditable at any future time.
