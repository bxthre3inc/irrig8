...
The Recursive Spawning Protocol (RSP) is the structural mechanism that converts the delegation chain from any active agent to a human principal from a forensic reconstruction — assembled after the fact from mutable logs and policy assertions — into a runtime-verifiable artifact that is immutable, cryptographically enforced, and architecturally verifiable at every spawn event. RSP operates across all three \bxthree{} layers in mandatory concert: the Purpose Layer serves as the exclusive authorization origin, the Bounds Engine enforces depth constraints as structural invariants, and the Fact Layer establishes the immutable parent pointer and maintains the tamper-evident forensic ledger. There are no advisory phases, no configurable constraints, and no bypass paths for any machine actor under any operational circumstance.

% --------------------------------------------------------------------------%
\subsection{Protocol Overview}
\label{sec:rs-overview}

RSP defines a mandatory five-phase execution sequence for every spawn event. Each phase must complete successfully before the next begins; any failure terminates the spawn attempt. The five phases are:

\begin{enumerate}
    \item Purpose Layer validation gate — authorization and scope refinement warrant
    \item Bounds Engine depth check — architectural depth bound enforcement
    \item Fact Layer immutable pointer creation and forensic ledger recording
    \item Child node activation with immutable parent reference
    \item Chain verification — structural termination at human principal
\end{enumerate}

This sequence is not a recommended workflow — it is a structural execution order enforced by the Fact Layer pre-commit hook, which blocks any attempt to bypass any phase. The protocol is deterministic: given identical inputs, RSP produces identical outcomes regardless of the operational state, urgency, or intentions of any actor in the system.

% --------------------------------------------------------------------------%
\subsection{Phase 1: Purpose Layer Validation}
\label{sec:rs-purpose-layer}

Every spawn event begins with the Purpose Layer, which serves as the exclusive authorization origin for chain growth in the RSP architecture. The Purpose Layer is not a policy layer — it is a structural gate whose output is a cryptographically signed warrant without which no spawn can proceed.

\medskip
\noindent\textbf{Authorization conditions.} A parent node $n_i$ that intends to spawn a child $n_{i+1}$ must submit a spawn request to the Purpose Layer declaring: the proposed child operating scope $R(n_{i+1})$, the specific operational condition necessitating spawning, and a demonstration that the condition cannot be resolved within $R(n_i)$. The Purpose Layer validates three mandatory conditions before issuing a warrant:
...
The warrant is the sole authorized precondition for spawn. No machine actor may initiate a spawn event without a matching warrant in the Fact Layer ledger — the Fact Layer pre-commit hook rejects any provisioning request that lacks a valid warrant before the operation reaches the ledger write stage.
...
% --------------------------------------------------------------------------%
\subsection{Phase 4: Child Activation with Immutable Parent Reference}
\label{sec:rs-child-activation}
...
% --------------------------------------------------------------------------%
\subsection{Phase 5: Chain Verification — Termination at Human Principal}
\label{sec:rs-verification}
...
% --------------------------------------------------------------------------%
\subsection{Depth Management: Maximum Depth, Refusal, and Convergence}
