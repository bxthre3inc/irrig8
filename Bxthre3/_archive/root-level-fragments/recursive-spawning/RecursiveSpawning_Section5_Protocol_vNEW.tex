...
The Recursive Spawning Protocol (RSP) is the structural mechanism that enforces an immutable, auditable, and architecturally verifiable delegation chain from every active agent to a human principal. RSP replaces the forensic reconstruction of delegation provenance — assembled after the fact from mutable logs and policy assertions — with a runtime artifact that is verified at every spawn event without exception. The protocol operates across all three \bxthree{} layers in mandatory concert: the Purpose Layer authorizes, the Bounds Engine constrains, and the Fact Layer records and enforces. There are no advisory phases, no configurable bypasses, and no operational circumstances under which any machine actor may circumvent any protocol requirement.

% --------------------------------------------------------------------------
\subsection{Protocol Architecture}
\label{sec:rs-arch}

RSP defines a mandatory five-phase execution sequence for every spawn event. Each phase must complete successfully before the next begins; any phase failure terminates the spawn attempt with a ledger entry. The five phases are:
...
This sequence is not a recommended workflow. It is a structural execution order enforced by the Fact Layer pre-commit hook, which blocks any attempt to bypass any phase. The protocol is deterministic: given identical inputs, RSP produces identical outcomes regardless of operational state, urgency, or the intentions of any actor in the system.

% --------------------------------------------------------------------------
\subsection{Phase 1: Purpose Layer Validation}
\label{sec:rs-purpose-validation}

Every spawn event begins with the Purpose Layer, which serves as the exclusive authorization origin for chain growth. The Purpose Layer is not a policy layer — it is a structural gate whose output is a cryptographically signed warrant without which no spawn can proceed.
...
% --------------------------------------------------------------------------
\subsection{Depth Management: Refusal and Convergence}
