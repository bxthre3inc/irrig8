...
For any RSP chain $\Chain{a}$ rooted at $n_0$ with $\alpha(n_0) = \bot$ and $h(n_0)$ designated, the human anchor $h(n_0) \in \Chain{a}$ and no node in $\Chain{a}$ is a machine actor. The chain is always non-empty for non-root nodes and always terminates at a named human.
\end{corol}
...
The Fact Layer write protocol is the structural mechanism that enforces the immutability of $\ParentPointer{p}{c}$. Immutability is not a permissions setting layered over a read/write interface; it is a property of the protocol's operation definitions. The Fact Layer physically cannot execute any write to an established $\ParentPointer{p}{c}$.

The write protocol defines four properties that collectively constitute protocol-level immutability:
...
While existing $\ParentPointer{p}{c}$ relations are immutable, the system must be able to grow the accountability chain through new spawn events. The creation of a new $\ParentPointer{p}{c}$ is the sole mechanism by which the chain extends. This creation requires explicit, Purpose-Layer-mediated authorization: no node may spontaneously generate a child and thereby establish a new parent pointer without going through the authorized spawn protocol.

The Purpose Layer plays two structurally necessary roles in the parent pointer lifecycle:

\begin{enumerate}
    \item[(A1) \textbf{Policy origination.}] At system initialization and at each chain extension, the Purpose Layer defines the spawn authorization policy $P_{\mathrm{spawn}}$ and records it in the Fact Layer authorization ledger. $P_{\mathrm{spawn}}$ specifies the mandatory conditions for any valid spawn event: (S1) the proposed child node $c$ does not already exist in $\mathcal{N}$; (S2) the authorized scope $\sigma_c$ is a descendant refinement of $R(p)$, the parent's operating scope; (S3) the spawn does not exceed the depth bound $D_{\max}$ for the proposed parent $p$.

    \item[(A2) \textbf{Warrant issuance.}] The Purpose Layer issues the cryptographic spawn warrant $\phi$ over the spawn request, which is the prerequisite for the Fact Layer provisioning write. The warrant $\phi$ is a Purpose-Layer signature over the 5-tuple $\mathrm{spawn}(c, p, \sigma_c, t, \phi)$, binding the child, parent, scope, timestamp, and Purpose Layer identity into a single non-forgeable authorization artifact.
\end{enumerate}

\begin{prop}[Warrant-Pointer Correspondence]
\label{prop:warrant-pointer}
For any spawn event $\mathrm{spawn}(c, p, \sigma_c, t, \phi)$, the Fact Layer recorded $\ParentPointer{p}{c}$ atomically at the time of its creation. Conversely, every $\ParentPointer{p}{c}$ established in the Fact Layer corresponds to a Purpose Layer warrant $\phi$ created at the same atomic write. No parent pointer exists without a warrant, and no warrant exists without a corresponding parent pointer.
\end{prop}

\begin{prop}[Purpose Layer Authorization Necessity]
\label{prop:auth-necessity}
No node $c \in \mathcal{N}$ may have $\ParentPointer{p}{c}$ established without a valid Purpose Layer warrant $\phi$ satisfying conditions (S1), (S2), and (S3). A spawn event that fails any of these conditions is rejected by the Purpose Layer pre-commit hook before the Fact Layer provisioning write is attempted. The pre-commit hook is the structural mechanism that prevents unauthorized chain growth at the point of spawn --- not a documentation mechanism that records chain growth after the fact.
\end{prop}
