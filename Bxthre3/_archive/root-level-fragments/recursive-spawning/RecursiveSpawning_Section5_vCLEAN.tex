...
The Recursive Spawning Protocol (RSP) is the structural mechanism that converts the delegation chain from a mutable runtime artifact — reconstructable only through forensic analysis of event logs — into an immutable, cryptographically enforced, runtime-verifiable accountability record. RSP operates across all three \bxthree{} layers in mandatory concert: the Purpose Layer authorizes, the Bounds Engine constrains, and the Fact Layer records and enforces. There are no advisory phases, no configurable bypasses, and no operational circumstances under which any machine actor may circumvent any protocol requirement. RSP is defined for a system $\mathcal{S} = \langle \mathcal{N}, \alpha, R, h, \mathcal{L}, D_{\max} \rangle$ where $\mathcal{N}$ is the agent node set, $\alpha(n)$ is the immutable parent pointer, $R(n)$ is the authorized operating scope, $h(n)$ is the human anchor, $\mathcal{L}$ is the Fact Layer forensic ledger, and $D_{\max}$ is the architectural maximum spawn depth.

% -----------------------------------------------------------------------
\subsection{Phase 1: Purpose Layer Validation Gate}

Every spawn event begins with the Purpose Layer, which serves as the exclusive authorization origin for chain growth. The Purpose Layer is not a policy convention — it is a structural gate whose output is a cryptographically signed warrant without which no spawn can proceed. A parent node $n_i \in \mathcal{N}$ that intends to spawn a child $n_{i+1}$ must submit a spawn request declaring: the proposed child operating scope $R(n_{i+1})$, the specific operational condition necessitating spawning, and a demonstration that the condition cannot be resolved within $R(n_i)$.

\medskip
\noindent\textbf{Validation conditions.} The Purpose Layer validates three mandatory conditions before issuing a warrant:
