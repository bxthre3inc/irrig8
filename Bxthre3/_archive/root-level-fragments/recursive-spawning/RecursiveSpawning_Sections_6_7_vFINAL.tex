...
The Recursive Spawning Protocol is a \emph{structural instantiation} of the BX3 three-layer accountability architecture, not a supplementary module composed alongside it. Every RSP mechanism maps onto exactly one BX3 layer; each mapping is minimal and structurally exclusive. There are no advisory phases, no configurable bypasses, and no operational circumstances under which any machine actor may circumvent any protocol requirement. The three mappings are: Purpose Layer as exclusive accountability anchor, Bounds Engine as depth enforcement, and Fact Layer as forensic ledger. No proper subset of these layers achieves the same guarantees.
