...
\medskip
\noindent\textbf{Definition 1 (Orphaned Agent).} An agent $n$ is \emph{orphaned} if its parent node $\alpha(n)$ has either (a) \emph{terminated} — exited operational state through normal completion, abnormal failure, or administrative shutdown — or (b) \emph{become unreachable} — non-responsive beyond a defined timeout $\tau_{\mathrm{heartbeat}}$. The agent persists in operational state but is disconnected from its chain of authorization.
