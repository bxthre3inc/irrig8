...
Before specifying the Recursive Spawning Protocol, we must precisely characterize the failure mode it targets: autonomous drift — the condition in which an agent continues to operate with no traceable authorization path to a human principal. This section provides formal definitions, identifies the structural conditions that enable drift, catalogs concrete failure modes, and proves that depth limits alone are categorically insufficient to prevent it.

% -------------------------------------------------------------------------
\subsection{Definitions and Formal Characterization}

We model a multi-agent system as a rooted directed acyclic graph (DAG) of agent nodes. Each node $n$ carries a parent pointer $\alpha(n)$ — the node that authorized its provisioning. The root of every authorized chain is a human principal $h \in \mathcal{H}$. A spawn chain $C = \langle n_0, n_1, \ldots, n_k \rangle$ satisfies $n_{i-1} = \alpha(n_i)$ for all $1 \leq i \leq k$ and $n_0 \in \mathcal{H}$.
...
The four possible combinations of the two binary sub-modes (orphaned, drifted) and operating state are given in Table~\ref{tab:drift-submodes}.
