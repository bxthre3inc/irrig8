...
A parent node $n_i \in \mathcal{N}$ that intends to spawn a child $n_{i+1}$ must submit a spawn request declaring: (P1) the proposed child operating scope $R(n_{i+1})$, (P2) the specific operational condition that necessitates spawning, and (P3) a demonstration that the condition cannot be resolved within $R(n_i)$. The Purpose Layer validates three mandatory conditions before issuing a warrant:
