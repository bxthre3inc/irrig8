...
\medskip
\noindent\textbf{Definition 2 (Drifted Agent).} An agent $n$ is \emph{drifted} if its delegation chain $C(n) = \langle n_0, n_1, \ldots, n \rangle$ does not contain a human principal at its origin $n_0$. Equivalently: there exists no human anchor $h$ such that the chain terminates at $n_0 = h$. A drifted agent may or may not be orphaned — it may have a living parent, but that parent's authority does not trace to any human decision.
