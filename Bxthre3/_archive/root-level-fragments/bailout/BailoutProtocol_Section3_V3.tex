...
\bigskip
\noindent\textbf{Transition conditions.} The seven transitions of $\mathcal{B}$ are enumerated formally:

\begin{enumerate}\itemsep=0pt
\item[T1 --- \texttt{IDLE $\rightarrow$ BOUNDED}:] Fires when the Bailout Monitor's pre-commit hook evaluates $\text{TRIGGER}(n, x) = \text{true}$ for some input $x$ in the node's observable input space. Defined in \S~\ref{sec:bailout-trigger}.

\item[T2 --- \texttt{BOUNDED $\rightarrow$ IDLE}:] Fires when the \fact{} layer approves a resolution $r$ produced by the Bounds Engine before $T_{\text{bounds}}$ expires. The resolution is written to the authorization ledger as a \fact-layer-authorized entry and the protocol run terminates successfully.
...
\item[T4 --- \texttt{BAIL\_PENDING $\rightarrow$ ESCALATING}:] Fires immediately and atomically upon entry to BAIL\_PENDING. No dwell time is permitted. No machine actor may suppress, redirect, or delay this transition. The Bailout Monitor commits the transition to the Forensic Ledger as an atomic state change, then immediately invokes the escalation algorithm (Alg~\ref{alg:escalate}).

\item[T5 --- \texttt{ESCALATING $\rightarrow$ HUMAN\_ACKNOWLEDGED}:] Fires when $h(n)$ confirms receipt of the escalated exception via any provisioned functional pathway. The confirmation is recorded in the Forensic Ledger, the propagation path is sealed as complete, and the originating node enters HUMAN\_ACKNOWLEDGED awaiting a directive.

\item[T6 --- \texttt{ESCALATING $\rightarrow$ ESCALATING}:] Fires on each timeout event during propagation ($T_{\text{prop}}$ expiration). The algorithm increments the retry counter and re-selects a pathway from the Pathway Health Registry. The state remains ESCALATING; this transition records the retry event without advancing the protocol.

\item[T7 --- \texttt{ESCALATING $\rightarrow$ RESOLVED}:] Fires when the human anchor issues a binding directive ($e_{\text{resolve}}$ or $e_{\text{reject}}$) after acknowledgment, or when all pathways are exhausted after $N_{\max}$ retries at $T_{\text{cap}}$. The directive is recorded in the authorization ledger; the state becomes terminal.
\end{enumerate}
...
\Procedure{ForwardException}{$n$, $m$, $\text{ctx}$, path}
  \State $\text{ctx}_\text{next} \leftarrow \text{ctx} \cup \{\text{nodeId} = n,\; \text{hopTimestamp} = \text{now()}\}$
  \State \Call{SendToParent}{$m$, $\text{ctx}_\text{next}$, path}
\EndProcedure

\Procedure{SelectPathway}{PHR}
  \State $F \leftarrow \{p \in \text{PHR} \mid \text{p.status} \neq \texttt{DEGRADED}\}$
  \State \Return $\arg\!\min_{p \in F}\; \text{p.estimatedLatency}$
\EndProcedure

\Procedure{BuildDiagnosticContext}{$n$, trigger}
  \State \Return $\{$
    \texttt{originNodeId}: $n$,
    \texttt{exceptionType}: trigger.type,
    \texttt{operatingRange}: $R(n)$,
    \texttt{confidence}: $\text{confidence}(n, \text{trigger.input})$,
    \texttt{timestamp}: $\text{now()}$,
    \texttt{bailoutHistory}: \Call{GetBailoutHistory}{$n$}
  $\}$
\EndProcedure
\end{algorithmic}
\end{algorithm}
...
\bigskip
\noindent\textbf{Propagation semantics.} The algorithm maintains the following guarantees:

\begin{itemize}\itemsep=0pt
\item[\textbf{P1 (Path integrity).}] The exception always propagates along the parent chain from the originating node toward $h(n)$. No node may select an alternate destination for an active bailout.
\item[\textbf{P2 (Context augmentation).}] Each intermediate node appends its node identifier, local timestamp, and any additional diagnostic state to the propagation context before forwarding. This produces an immutable chain of custody record in the Forensic Ledger.
\item[\textbf{P3 (Non-interference).}] Intermediate nodes execute only \texttt{ForwardException}; they do not attempt local resolution, do not modify the exception payload, and do not halt propagation under any condition other than pathway exhaustion.
\item[\textbf{P4 (Bounded retry).}] Propagation retries are bounded by $N_{\max}$ at $T_{\text{cap}}$, ensuring that the algorithm terminates within a finite wall-clock window even when all pathways are degraded.
\end{itemize}

Upon successful delivery to $h(n)$, the human anchor's acknowledgment triggers T5, transitioning the originating node from ESCALATING to HUMAN\_ACKNOWLEDGED. The anchor then evaluates the diagnostic context and issues one of three directives: \texttt{ACK} (return to IDLE), \texttt{RESOLVE} (execute a specified binding resolution and return to IDLE), or \texttt{REJECT} (enter quiescent RESOLVED state).

% ---------------------------------------------------------------
\subsection{Bypass Mechanism: Why Bypasses All Machine Actors}\label{sec:bypass}
% ---------------------------------------------------------------

The Bailout Protocol's escalation algorithm includes a critical structural property that distinguishes it from all conventional escalation hierarchies:

\bigskip
\noindent\textbf{Bypass property (BP).}
\begin{equation}
\forall n \in \text{nodes},\; \text{state}(n) = \text{ESCALATING} \implies \text{current\_destination}(n) = h(n) \;\land\; h(n) \notin \text{machine actors}. \tag{BP}
\end{equation}

That is: for any node in ESCALATING state, the current destination is exactly and only $h(n)$, and $h(n)$ is never a machine actor. The bypassing property holds not by convention but by architectural constraint: every enforcement mechanism in the protocol is designed to prevent a machine actor from occupying the terminus of an active bailout.
...
The introduction of any machine actor between the originating node and $h(n)$ in the communication path creates three mutually independent failure modes, each of which alone is sufficient to break the upstream accountability guarantee:

\begin{enumerate}
\item[\textbf{Intercept.}] A machine actor could absorb the escalation, suppress the exception, and continue autonomous operation without ever reaching $h(n)$.
