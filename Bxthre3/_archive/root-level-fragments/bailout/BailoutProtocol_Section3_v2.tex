...
All transitions are recorded in the Forensic Ledger atomically before the state change is committed. This ensures that the ledger contains a complete, immutable history of every state the node has occupied, enabling post-hoc reconstruction of the protocol run from the ledger alone.
...
\begin{enumerate}
    ...
    \item \textbf{Wait for acknowledgment.} Await ACK from $\alpha(\text{current})$ before $T_{\text{prop}}$ expires. If ACK is received: if $\alpha(\text{current}) = h(n)$, record $e_{\text{ack}}$ and transition to HUMAN\_ACKNOWLEDGED (T5); otherwise, set $\text{current} \leftarrow \alpha(\text{current})$, append current node and its diagnostic context to $\text{chain}$, and goto Step 2.
...
\bigskip
\noindent\textbf{Intercept prevention.} A machine actor could theoretically attempt to absorb the escalation, suppress the exception, and continue autonomous operation without ever reaching $h(n)$. This is precisely the failure mode the Bailout Protocol is designed to prevent, and the two enforcement mechanisms above make this intercept impossible by architectural constraint rather than by operational policy. Specifically:

\begin{itemize}\itemsep=0pt
    \item The Fact Layer gate rejects any resolution attempt that would close the exception at an intermediate machine actor after TC has fired and $T_{\text{bounds}}$ has expired. No machine actor has write authority over the authorization ledger in this condition.
    \item The immutable parent-pointer architecture ensures that no machine actor can reroute the escalation path to terminate at itself. The human-root identifier is read-only and is not modifiable by any machine actor without explicit \purpose{} layer authorization, which requires human sign-off.
\end{itemize}

Conventional escalation hierarchies — including tiered oversight models that permit absorption at intermediate tiers, human-in-the-loop checkpoints that are advisory rather than compulsory, and watchdog agents that may resolve exceptions without mandatory human notification — treat human oversight as a high-tier option. The Bailout Protocol structurally forbids this option: $h(n)$ is not one option among many for terminating the escalation; it is the \emph{only} permissible terminus for every unresolved exception. The parent-chain bypass skips intermediate nodes not solely for efficiency but because their presence in the terminus would break the accountability chain by introducing machine actors as potential terminal nodes.
