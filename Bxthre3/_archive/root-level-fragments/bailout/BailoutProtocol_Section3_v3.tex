...
\item[$\rightarrow$] $\; \subseteq Q \times \Sigma \times Q$ is the deterministic transition relation defined by Table~\ref{tab:bailout-transition}.
...
Each transition $T_i$ in the state machine is governed by formal preconditions that must hold atomically at the time of transition firing. The Bailout Monitor evaluates all preconditions via the \fact{} layer's pre-commit hook before committing any state change to the Forensic Ledger. No machine actor may bypass, reorder, or suppress this evaluation.

\medskip
\noindent\textbf{Transition T1: IDLE $\rightarrow$ BOUNDED.}

\[
\exists x \in \text{inputs}(n) \;.\; \text{TRIGGER}(n, x) \;=\; \text{true}
\tag{T1}
\]

Precondition: An input $x$ has been observed at node $n$ that satisfies the bail-out trigger condition. The \bounds{} layer evaluates TC against $R(n)$, the provisioned confidence threshold $\tau$, and the node's resolution history. The trigger evaluation is deterministic and read-only; it produces a boolean result with no machine-actor discretion. Upon T1 firing, the exception record and diagnostic context are atomically committed to the Forensic Ledger, and the Bounds Engine resolution timer is started.

\medskip
\noindent\textbf{Transition T2: BOUNDED $\rightarrow$ HUMAN\_ACKNOWLEDGED.}

\[
e_{\text{auth}} \;=\; \text{true}
\tag{T2}
\]

Precondition: The Bounds Engine has produced a resolution $r$ for exception $x$ within $T_{\text{bounds}}$, and the \fact{} layer has verified $r$ against the authorization ledger and confirmed it is within the node's provisioned authority. The Fact Layer approval is recorded as a ledger entry before the state transition is committed. This transition represents the normal exception-absorption path: a condition that was initially outside $R(n)$ has been successfully resolved within the machine-only execution graph.

\medskip
\noindent\textbf{Transition T3: BOUNDED $\rightarrow$ BAIL\_PENDING.}

\[
e_{\text{timeout}} \;=\; \text{true} \;\lor\; \texttt{bailout\_signal}(x)
\tag{T3}
\]

Precondition: Either (a) $T_{\text{bounds}}$ has expired without a \fact-layer-approved resolution, or (b) the Bounds Engine has emitted an explicit \texttt{bailout\_signal} for exception $x$, recognizing it as unresolvable even though $x \in R(n)$. In both cases, the Bounds Engine has exhausted all provisioned resolution paths. The transition commits an atomic exception record to the Forensic Ledger with the reason for bailout initiation.

\medskip
\noindent\textbf{Transition T4: BAIL\_PENDING $\rightarrow$ ESCALATING.}

\[
e_{\text{timeout}} \;=\; \text{true}
\tag{T4}
\]

Precondition: The node is in BAIL\_PENDING and $T_{\text{bounds}}$ has expired (or $T_{\text{bounds}}$ expired as the prior transition). This transition is atomic: no machine-actor dwell time is permitted in BAIL\_PENDING. The Bailout Monitor immediately initiates the escalation algorithm (Algorithm~\ref{alg:escalate}) upon entering ESCALATING.

\medskip
\noindent\textbf{Transition T5: ESCALATING $\rightarrow$ IDLE.}
...
Precondition: The human anchor $h(n)$ has received the escalation, verified the diagnostic context, and issued an acknowledgment \texttt{ACK} indicating the exception is noted but no binding directive is required. The node returns to normal operation in IDLE, carrying the exception in its history for future reference but with no active protocol run in flight. The acknowledgment and any appended notes are recorded in the Forensic Ledger.

\medskip
\noindent\textbf{Transition T6: ESCALATING $\rightarrow$ HUMAN\_ACKNOWLEDGED.}
...
Precondition: A parent node in the escalation chain has received and forwarded the exception, and the parent has verified the exception's integrity against the Fact Layer hash chain. The parent acknowledges receipt so the originating node can exit ESCALATING. This transition is used when the human anchor is not the direct parent but is reachable via one or more intermediate nodes that forward the exception without local resolution.

\medskip
\noindent\textbf{Transition T7: ESCALATING $\rightarrow$ RESOLVED.}

\[
e_{\text{resolve}} \;=\; \text{true} \;\lor\; e_{\text{reject}} \;=\; \text{true}
\tag{T7}
\]

Precondition: The human anchor has issued a binding directive — either \texttt{RESOLVE} specifying a particular resolution to execute, or \texttt{REJECT} disallowing the proposed action and placing the node into a quiescent error state. The directive is recorded verbatim in the Forensic Ledger as the authoritative resolution of the exception. The RESOLVED state is terminal; no further transitions are defined from it.
...
\caption{Escalation Algorithm $\texttt{Escalate}(n, e, C)$}
\label{alg:escalate}
\begin{algorithmic}
