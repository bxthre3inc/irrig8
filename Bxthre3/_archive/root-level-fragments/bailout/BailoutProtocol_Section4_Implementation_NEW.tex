...
\item[(\textit{iii})] The \purpose{} layer evaluates the exception and issues one of three directives: \texttt{ACK} (acknowledge and resume normal operation), \texttt{RESOLVE} (execute a specified binding resolution), or \texttt{REJECT} (disallow the proposed action and enter a quiescent error state). No machine actor may issue, simulate, or substitute any of these directives. The state machine enters \texttt{HUMAN\_ACKNOWLEDGED} and awaits the directive; upon receipt, the directive is committed to the authorization ledger and the machine transitions to \texttt{RESOLVED}.
