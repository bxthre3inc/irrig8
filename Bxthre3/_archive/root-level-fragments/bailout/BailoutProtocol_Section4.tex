...
\item[(\textit{iv})] \textbf{Consequential authority.} The \purpose{} layer is the only layer that carries the authority to commit consequential decisions to the authorization ledger. This exclusivity makes it the only permissible terminus for the accountability chain: any attempt by a machine actor to close a protocol run without a ledger entry bearing a \purpose{}-layer-authorized directive fails the \fact{} Layer's pre-commit hook.
\end{itemize}
...
% --------------------------------------------------------------
\subsection{Bounds Engine: Compulsory Trigger Enforcement}
...
\item The evaluation of $\text{TRIGGER}(n, x)$ is driven by the Bailout Monitor's pre-commit hook, which intercepts the query and commits the result to the state machine before any agent-level code can act on the input. There is no intervening decision step at which a machine actor could suppress or reclassify the exception.
