...
\begin{itemize}\itemsep=0pt
\item \textbf{Determinism.} The state machine requires \emph{deterministic} transition enforcement: for every $(q, \sigma) \in Q \times \Sigma$, at most one transition is defined. This determinism is delivered by the Bailout Monitor's priority function $\pi$, which totally orders simultaneous trigger conditions before committing any state change. The Fact Layer provides the scheduling guarantee and atomic pre-commit hook required to make this ordering consistent across all nodes.

\item \textbf{Atomicity.} The state machine requires \emph{atomic} state transitions: no intermediate or partially-executing state is observable externally. The Fact Layer's pre-commit hook guarantees this atomicity by evaluating and committing the transition in a single synchronized operation, preventing any interleaving machine actor from observing or modifying the state mid-transition.

\item \textbf{Parent observability.} The state machine requires that its state be \emph{observable} by the parent node and by the Bailout Monitor --- enabling parent-level monitoring of child escalation status without requiring access to the child node's internal state. The Fact Layer exposes a state observation interface to the parent node, enabling the parent to monitor $\mathcal{B}$'s current state without granting write access.

\item \textbf{Mandatory ledger append.} The state machine requires that every transition generate a Forensic Ledger entry. The Fact Layer's ledger write authority is exercised as a mandatory side effect of every successful transition: no state change in $\mathcal{B}$ is committed without a corresponding ledger entry recording the transition event, the triggering condition, and the diagnostic context.
\end{itemize}
