...
\item[\textbf{T4: BAIL\_PENDING $\rightarrow$ ESCALATING (protocol activation).}] Transition T4 fires immediately upon entry to BAIL\_PENDING — there is no dwell time, no retry attempt, and no machine-actor suppression option in this state. Formally:
\begin{equation}
\text{T4 fires} \iff \text{state} = \text{BAIL\_PENDING}. \tag{T4}
\end{equation}
This transition is unconditional upon entry. The protocol activates propagation to $h(n)$ as a single atomic action executed by the Bailout Monitor.

\item[\textbf{T5: ESCALATING $\rightarrow$ HUMAN\_ACKNOWLEDGED (anchor receipt).}] Transition T5 fires when the exception propagated by node $n$ reaches the human anchor $h(n)$ and $h(n)$ acknowledges receipt. For the originating node, receipt by $h(n)$ is terminal. For intermediate nodes in the propagation chain, receipt by $h(n)$ is observed as a terminal ACK that propagates back through the chain. Formally:
\begin{equation}
\text{T5 fires} \iff \text{directive} = \texttt{ACK} \land \text{state} = \text{ESCALATING}. \tag{T5}
\end{equation}

\item[\textbf{T6: ESCALATING $\rightarrow$ IDLE (false positive).}] Transition T6 fires when the human anchor receives the exception, reviews the diagnostic context, and determines that the exception was a false positive — the proposed action was within the node's authority and requires no directive. The anchor issues \texttt{ACK}, the state returns to IDLE, and the node resumes normal operation without protocol intervention. Formally:
\begin{equation}
\text{T6 fires} \iff \text{directive} = \texttt{ACK} \land \text{state} = \text{ESCALATING}. \tag{T6}
\end{equation}
Note that T5 and T6 share the same trigger ($e_{\text{ack}}$ in ESCALATING state); the distinction is the human anchor's intent as recorded in the directive metadata.

\item[\textbf{T7: ESCALATING $\rightarrow$ RESOLVED (binding resolution from escalation).}] Transition T7 fires when the human anchor issues a \texttt{RESOLVE} or \texttt{REJECT} directive while the node is in ESCALATING state. A \texttt{REJECT} records an error tag indicating the proposed action was disallowed; a \texttt{RESOLVE} records the binding resolution to be executed. Formally:
\begin{equation}
\text{T7 fires} \iff \text{directive} \in \{\texttt{RESOLVE}, \texttt{REJECT}\} \land \text{state} = \text{ESCALATING}. \tag{T7}
\end{equation}

\item[\textbf{T8: HUMAN\_ACKNOWLEDGED $\rightarrow$ IDLE (continue).}] Transition T8 fires when the human anchor issues \texttt{ACK} from HUMAN\_ACKNOWLEDGED state. The node returns to normal operation, incorporating any advisory parameters provided by the anchor. Formally:
\begin{equation}
\text{T8 fires} \iff \text{directive} = \texttt{ACK} \land \text{state} = \text{HUMAN\_ACKNOWLEDGED}. \tag{T8}
\end{equation}

\item[\textbf{T9: HUMAN\_ACKNOWLEDGED $\rightarrow$ RESOLVED (binding resolution from acknowledgment).}] Transition T9 fires when the human anchor issues a binding \texttt{RESOLVE} or \texttt{REJECT} directive from HUMAN\_ACKNOWLEDGED state. The resolution is recorded in the authorization ledger and the protocol run terminates. Formally:
\begin{equation}
\text{T9 fires} \iff \text{directive} \in \{\texttt{RESOLVE}, \texttt{REJECT}\} \land \text{state} = \text{HUMAN\_ACKNOWLEDGED}. \tag{T9}
\end{equation}
\end{enumerate}
...
\Procedure{Escalate}{$n$, trigger}
  ...
  \Repeat
    ...
    \If{$\text{acked} = \texttt{ACK}$}
      \State \Call{UpdateLedger}{\text{propagation\_success}, $n$, $p$, $\text{clock}$}
    ...
    \State \Call{UpdateLedger}{\text{retry}, $n$, $p$, $\text{attempts}$, $T_{\text{prop}}$, $\text{clock}$}
    \If{$\text{attempts} \geq N_{\max}$}
      \State \Call{FireTimeout}{$n$, ctx, \texttt{parent\_unreachable}}
...
\Procedure{ForwardException}{$n$, ctx}
  \State $\text{record.node\_id} \leftarrow n.\text{id}$
  \State $\text{record.timestamp} \leftarrow \text{clock}$
  \State $\text{record.node\_state\_snapshot} \leftarrow n.\text{snapshot}()$
  \State $\text{record.local\_diagnostics} \leftarrow n.\text{bounds\_engine.context\_summary}$
  \State $\text{ctx.propagation\_chain.append}(\text{record})$
  \State \Call{UpdateLedger}{\text{forward}, $n$, $\text{record}$, $\text{clock}$}
  \State $p \leftarrow \alpha(n)$
  \State \Call{SendToParent}{$p$, ctx, $T_{\text{prop}}$, \Call{SelectPathway}{PHR, $p$}}
\EndProcedure
\\[6pt]
\Procedure{BuildDiagnosticContext}{$n$, trigger}
  \State $\text{ctx} \leftarrow \{\}$
  \State $\text{ctx.event} \leftarrow \text{trigger.event}$
  \State $\text{ctx.node\_state} \leftarrow n.\text{snapshot}()$
  \State $\text{ctx.trigger\_condition} \leftarrow \text{trigger.condition}$
  \State $\text{ctx.confidence} \leftarrow \text{trigger.confidence}$
  \State $\text{ctx.severity} \leftarrow \text{trigger.severity}$
  \State $\text{ctx.bounds\_attempt} \leftarrow n.\text{bounds\_engine.last\_attempt}$
  \State $\text{ctx.resolution\_history} \leftarrow n.\text{bounds\_engine.resolution\_history}$
  \State $\text{ctx.fired\_at} \leftarrow \text{clock}$
