...
\item[\textbf{ESCALATING.}] The exception is propagating upward toward $h(n)$. Each intermediate node receives the exception, appends its diagnostic context to the propagation chain (detailed in \S~\ref{sec:escalation-algorithm}), and forwards the exception via \texttt{SendToParent} without attempting local resolution. The state remains ESCALATING until either (a) the human anchor acknowledges receipt ($e_{\text{ack}}$: T5), (b) the human anchor issues a binding directive ($e_{\text{resolve}}$ or $e_{\text{reject}}$: T7), or (c) all retry pathways are exhausted after $N_{\max}$ retries at $T_{\text{cap}}$.
...
Each transition T1–T9 encodes a precise condition on the protocol state and the \bxthree{} layer interfaces:

\begin{itemize}\itemsep=0pt
\item[T1 --- \texttt{IDLE $\rightarrow$ BOUNDED}:] Fires when $\text{TRIGGER}(n, x)$ holds for some $x$ in the node's observable input space. This is the entry point for all bailout activations; no transition may skip directly to ESCALATING or HUMAN\_ACKNOWLEDGED without passing through BOUNDED.
...
All transitions are recorded in the Forensic Ledger with the originating state, the input event, the resulting state, and a timestamp with millisecond precision. The ledger entry is cryptographically sealed with a SHA-256 hash chain linking it to its predecessor, making retrospective tampering computationally detectable. This recording is not optional: it is enforced by the \fact{} layer pre-commit hook, and no machine actor — including the \bounds{} layer, the Bounds Engine, or any agent-level component — may suppress, modify, or interfere with a transition recording.
...
\bigskip
\noindent\textbf{The no-discretion property.} The \bounds{} layer does not exercise discretion over the trigger condition: (1) The evaluation of $\text{TRIGGER}(n, x)$ is a read-only query against the operating range definition $R(n)$, not a judgment call made by a machine actor. (2) The result of the query directly drives the state machine transition without an intervening decision step; there is no machine-actor gate between trigger evaluation and protocol activation. (3) The confidence function is provisioned at node initialization and is immutable at runtime; no machine actor may adjust $\tau$ to suppress escalation. These constraints ensure that the trigger condition is architecturally enforced rather than operationally optional.
...
\begin{enumerate}
    ...
    \item \textbf{Wait for acknowledgment.} Await ACK from $\alpha(\text{current})$ before $T_{\text{prop}}$ expires. If ACK is received: record the hop in the chain, update the PHR latency estimate, and if $\alpha(\text{current}) = h(n)$, transition to HUMAN\_ACKNOWLEDGED. Otherwise, set $\text{current} \leftarrow \alpha(\text{current})$ and goto Step 2.

    \item \textbf{Timeout on parent.} If $T_{\text{prop}}$ expires without ACK: increment $\text{attempt}$. If $\text{attempt} < N_{\max}$, double $T_{\text{prop}}$ (capped at $T_{\text{cap}}$) and retry the same pathway. If $\text{attempt} \geq N_{\max}$, record \texttt{PARENT\_UNREACHABLE} in the Forensic Ledger and attempt delivery via an alternate functional pathway tracked in the Pathway Health Registry. If no alternate pathway is available after exhausting all provisioned pathways, terminate with RESOLVED and tag \texttt{PATHWAY\_EXHAUSTED} — this is a terminal state with a recorded forensic tag, not a suppression of escalation.
...
\caption{Escalation algorithm \texttt{Escalate}$(n, e)$}
\label{alg:escalate}
\begin{algorithmic}[1]
\State $\text{current} \leftarrow n$, $\text{chain} \leftarrow []$, $\text{attempt} \leftarrow 0$
\State $\text{Append}(n, \text{diag}(n)) \to \text{chain}$
\State $\text{Record}(\texttt{ESCALATING\_START}, n, e)$
\While{$\text{current} \neq h(n)$}
    \State $p \leftarrow \text{SelectPathway}(\alpha(\text{current}), \text{PHR})$
    \State $\text{Send}(e, \text{chain})$ via $p$
    \State $\text{StartTimer}(T_{\text{prop}})$
    \State \textbf{await} ACK \textbf{or} $T_{\text{prop}}$ \textbf{expiry}
    \If{ACK received}
        \State $\text{Append}(\alpha(\text{current}), \text{diag}(\alpha(\text{current}))) \to \text{chain}$
        \State $\text{UpdatePHR}(p, \text{latency})$
        \State $\text{current} \leftarrow \alpha(\text{current})$
    \Else
        \State $\text{attempt} \leftarrow \text{attempt} + 1$
        \If{$\text{attempt} \geq N_{\max}$}
            \State $p_{\text{alt}} \leftarrow \text{SelectAlternate}(e, \text{PHR})$
            \If{$p_{\text{alt}} = \emptyset$}
                \State $\text{Record}(\texttt{PATHWAY\_EXHAUSTED})$
                \State \textbf{return} RESOLVED
            \EndIf
            \State $p \leftarrow p_{\text{alt}}$, $\text{attempt} \leftarrow 0$
        \Else
            \State $T_{\text{prop}} \leftarrow \min(2 \cdot T_{\text{prop}},\; T_{\text{cap}})$
        \EndIf
    \EndIf
\EndWhile
\State $\text{Record}(\text{HUMAN\_ACK}, h(n), \text{chain})$
\State \textbf{await} directive $\in \{\texttt{RESOLVE}, \texttt{REJECT}, \texttt{ACK}\}$
\If{directive = \texttt{RESOLVE} \textbf{or} \texttt{REJECT}}
    \State $\text{Record}(\text{directive}, \text{ledger})$
    \State \textbf{return} RESOLVED
\Else
    \State \textbf{return} IDLE
\EndIf
\end{algorithmic}
\end{algorithm}
...
\bigskip
\noindent\textbf{Intercept.} A machine actor could attempt to absorb the escalation, suppress the exception, and continue autonomous operation without ever reaching $h(n)$. This is precisely the failure mode the Bailout Protocol is designed to prevent, and the two enforcement mechanisms above make this intercept impossible by architectural constraint rather than by operational policy.
