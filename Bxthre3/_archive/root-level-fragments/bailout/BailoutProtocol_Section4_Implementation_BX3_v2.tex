...
The human anchor $h(n)$ is not a design option. The functional separation between \purpose{}, \bounds{}, and \fact{} layers establishes that the \purpose{} layer is the only layer carrying both intent and final accountability. The Bounds Engine detects exception conditions but carries no judgment authority; the \fact{} layer enforces transition relations and records events but carries no intent authority. When an exception exceeds the Bounds Engine's provisioned scope, no machine actor exists that has the authority to adjudicate it. The Bailout Protocol is the mechanism by which this structural necessity is enforced at runtime.
...
This atomic transition is the architectural guarantee that the bailout trigger is compulsory. The \fact{} Layer's pre-commit hook enforces the transition relation directly, bypassing any agent-level code that might otherwise interpose.
...
\begin{align}
\text{INV-2} &: \text{state}(n) = \text{BAIL\_PENDING} \implies \text{active\_bailout}(n) \land \text{transition}(\text{BAIL\_PENDING}, e_{\text{timeout}}) = \text{ESCALATING}. \tag{INV-2}
\end{align}
