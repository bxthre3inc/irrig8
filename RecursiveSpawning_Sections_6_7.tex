...
\begin{enumerate}
  ...
  \item \textbf{Scope Bound ($S_{\max}$)}. The maximum number of simultaneous active sub-agents a parent may have at any time. A spawn that would exceed $S_{\max}$ is refused with \texttt{SCOPE-LIMIT-REACHED}.
  \item \textbf{Authority Bound}. A bitmask defining permitted action classes. A child cannot receive authority its parent does not possess --- the \textbf{authority monotonicity} constraint.
  \item \textbf{Temporal Bound}. A TTL after which the agent session expires regardless of completion status, recorded as \texttt{AGENT-EXPIRED} in the Fact Layer.
\end{enumerate}

Critically, the Bounds Engine's scope check is independent of the Purpose Layer's projection function. If the Purpose Layer's scope analysis were compromised, the Bounds Engine's independent computation provides a second enforcement layer that prevents scope violations from bypassing detection. Both layers must approve before a spawn proceeds. This two-party authorization model ensures that the correctness of spawn authorization does not depend on any single layer remaining honest --- a compromise of either the Purpose Layer or the Bounds Engine alone is insufficient to produce an unauthorized spawn.
...
For Recursive Spawning specifically, the Forensic Ledger records four event types relevant to drift prevention: \texttt{child-provisioned}$(p, c, R(c), h(c))$, encoding the immutable parent pointer $\alpha(c) = p$ and the human anchor $h(c)$; \texttt{spawn-refused}$(p, c, \text{reason})$, recording a rejected spawn with its violation reason; \texttt{bailout-triggered}$(n, e)$, recording activation of the Bailout Protocol; and \texttt{parent-pointer-write-denied}$(v, \alpha_{\text{old}}, \alpha_{\text{new}})$, recording that a machine actor attempted to modify the parent pointer and was rejected. These four event types exhaustively cover every state transition relevant to chain integrity and drift detection.
...
These properties are not configurable, not discretionary, and not bypassable. Their enforcement is distributed across three independent BX3 layers, making the drift-prevention guarantee robust against single-point compromise. The conjunction of all three is required --- none is sufficient alone.
