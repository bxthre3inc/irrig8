...
\label{tab:bp-events}
\medskip
\noindent\begin{tabular}{lp{8.0cm}}
\toprule
...
T7 & \texttt{ESCALATING}            & $\sigma_{\text{cap}}$ or all pathways exhausted                      & \texttt{PATHWAY\_EXHAUSTED} \\
...
T10 & \texttt{HUMAN\_ACKNOWLEDGED}   & $\sigma_{\text{timeout}}(\tau_{\text{human}})$                         & \texttt{TIMEOUT} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}\itemsep=0pt
\item[$\textbf{T1 (IDLE} \rightarrow \textbf{BOUNDED):}$] Transition T1 fires when the trigger condition TC evaluates to \texttt{true} at node $n$ (Section~\ref{sec:bp-trigger}), or when the Bounds Engine emits an explicit \texttt{bailout\_signal}. The pre-commit gate blocks all consequential action at $n$ upon this transition and arms $\tau_{\text{bounds}}$.

\item[$\textbf{T2 (BOUNDED} \rightarrow \textbf{IDLE):}$] The human anchor $h(n)$ issues an acknowledgment directive $\sigma_{\text{resolve}}(\texttt{ACK})$. This is the normal-resolution path: the Bounds Engine's candidate resolution is approved by the human principal, and operation resumes normally at $n$. Pre-commit enforcement: credential verification against $C_{h(n)}$.

\item[$\textbf{T3 (BOUNDED} \rightarrow \textbf{BAIL\_PENDING):}$] The timeout $\tau_{\text{bounds}}$ expires before any resolution approval is recorded. The Bounds Engine has failed to produce an admissible resolution within the allocated budget. Pre-commit: the \fact{} layer verifies that no resolution approval was recorded before expiration; the \texttt{bailout\_signal} is forwarded to the Bailout Monitor.

\item[$\textbf{T4 (BAIL\_PENDING} \rightarrow \textbf{ESCALATING):}$] The propagation preparation timeout $\tau_{\text{prop}}$ expires. The system has failed to establish a viable propagation context (parent-pointer verification, pathway health check, or diagnostic assembly) within the allocated budget. Pre-commit: a \texttt{bail-pending-timeout} entry is recorded; the state machine enters \texttt{ESCALATING} unconditionally.

\item[$\textbf{T5 (ESCALATING} \rightarrow \textbf{ESCALATING):}$] This is a self-loop transition. The \texttt{SendToParent} operation succeeds at the current hop and returns acknowledgment within $\tau_{\text{prop}}$. The propagation chain advances one step toward $h(n)$. Pre-commit: an \texttt{escalation-hop} entry records the pathway identifier, payload hash, and hop count.

\item[$\textbf{T6 (ESCALATING} \rightarrow \textbf{HUMAN\_ACKNOWLEDGED):}$] The propagation chain reaches the anchor node $h(n)$ and the human anchor acknowledges receipt via $\sigma_{\text{human\_ack}}$. Pre-commit: the \fact{} layer verifies the acknowledgment originates from $h(n)$ using credential verification against $C_{h(n)}$; a \texttt{human-ack} entry is recorded.

\item[$\textbf{T7 (ESCALATING} \rightarrow \textbf{PATHWAY\_EXHAUSTED):}$] Either the cascade limit $D_{\max}$ is reached during propagation (the chain has traversed $D_{\max}$ nodes without reaching a human anchor), or all candidate pathways in the Pathway Health Registry have been exhausted. This is a terminal failure state. Pre-commit: a \texttt{pathway-exhausted} entry records the complete propagation chain attempted and all failed pathway identifiers.

\item[$\textbf{T8 (HUMAN\_ACKNOWLEDGED} \rightarrow \textbf{RESOLVED):}$] $\sigma_{\text{resolve}}(d)$ is received from $h(n)$ where $d \in \{\,\texttt{RESOLVE},\ \texttt{REJECT},\}$. \texttt{RESOLVE} prescribes a specific corrective action; \texttt{REJECT} disallows the proposed action and enters a quiescent error state. Pre-commit: credential verified against $C_{h(n)}$; a \texttt{human-directive} entry records the directive type and authorization ledger hash.

\item[$\textbf{T9 (HUMAN\_ACKNOWLEDGED} \rightarrow \textbf{IDLE):}$] $\sigma_{\text{resolve}}(\texttt{ACK})$ is received from $h(n)$, returning the node to normal operation. Pre-commit: credential verified against $C_{h(n)}$.

\item[$\textbf{T10 (HUMAN\_ACKNOWLEDGED} \rightarrow \textbf{TIMEOUT):}$] The human acknowledgment timeout $\tau_{\text{human}}$ expires while in \texttt{HUMAN\_ACKNOWLEDGED}. This is a terminal failure state. Pre-commit: a \texttt{timeout-human} entry records the elapsed time and last known state.
\end{enumerate}
...
\Function{Escalate}{$n$}
  ...
  \Comment Full context: trigger components, Bounds Engine reasoning trace, resolution history
  ...
  \While{$\neg\ \text{IsAnchor}(n_{\text{current}})$}
    \If{$\text{hops} \geq D_{\max}$}
      ...
      \Comment T7: cascade limit reached
    \EndIf

    \State $\text{parent} \gets \alpha(n_{\text{current}})$
    \Comment Read-only; enforced immutable by \fact{} layer pre-commit hook

    \State $\text{selected} \gets \Call{SelectPathway}{P, \text{parent}}$
    \If{$\text{selected} = \texttt{none}$}
      \State \Return \Call{TransitionTo}{\texttt{PATHWAY\_EXHAUSTED}}
      \Comment T7: no functional pathway remains
    \EndIf

    \State $(\text{ok}, \text{ack}) \gets \Call{SendToParent}{\text{parent}, \text{payload}, \text{selected}}$
    \If{$\text{ok}$}
      \State $\text{payload} \gets \text{ExtendDiagnosticChain}(\text{payload}, n_{\text{current}})$
      \State $n_{\text{current}} \gets \text{parent}$
      \State $\text{hops} \gets \text{hops} + 1$
    \Else
      \State $\Call{MarkPathwayDegraded}{P, \text{selected}}$
      \State \Return \Call{TransitionTo}{\texttt{PATHWAY\_EXHAUSTED}}
      \Comment T7: parent unreachable via all candidate pathways
    \EndIf
  \EndWhile

  \State \Comment{$n_{\text{current}}$ is now $h(n)$}
  \State \Return \Call{TransitionTo}{\texttt{HUMAN\_ACKNOWLEDGED}}
  \Comment T6: propagation chain reaches anchor
\EndFunction
...
\Function{SelectPathway}{$P, v$}
  ...
  \Comment Select highest-priority functional parent at $v$, excluding \texttt{degraded} pathways
\EndFunction
\end{algorithmic}
\end{algorithm}

The diagnostic chain assembled at each hop records the state of the originating node, the Bounds Engine's reasoning trace at each intermediate node during \texttt{BOUNDED}, and the timeout states that drove propagation. This chain is assembled by the \fact{} layer, not by any machine actor, ensuring completeness and preventing selective omission.

The escalation algorithm has a strict termination bound. The parent-pointer chain from any node $n$ is a strictly descending sequence in the depth ordering (each step moves to a node with lower depth), and the chain terminates at the anchor $h(n)$. The maximum number of iterations is $d(n) - d(h(n)) \leq D_{\max}$, a finite constant established at framework initialization. The chain cannot cycle because self-referential parentage is prevented at node creation by the \fact{} layer's pre-commit hook.

\begin{table}[t]
\caption{Bailout Protocol Timeout Parameters}
\label{tab:bp-timeouts}
\medskip
\noindent\begin{tabular}{lp{3cm}p{5.5cm}}
\toprule
\textbf{Parameter} & \textbf{Default} & \textbf{Description} \\
\midrule
$\tau_{\text{bounds}}$ & $T_{\text{init}}$ & Time granted to the Bounds Engine to produce and gain approval for a candidate resolution during \texttt{BOUNDED}. Expiration $\rightarrow$ T3. \\
$\tau_{\text{prop}}$ & $T_{\text{prop}}$ & Time allowed for each \texttt{SendToParent} invocation to receive acknowledgment. Expiration $\rightarrow$ T4 or T7. \\
$\tau_{\text{cap}}$ & $D_{\max}$ hops & Maximum propagation depth before \texttt{PATHWAY\_EXHAUSTED} is entered. Measured in node hops, not wall time. \\
$\tau_{\text{human}}$ & $T_{\text{human}}$ & Time allowed for $h(n)$ to acknowledge and issue a directive in \texttt{HUMAN\_ACKNOWLEDGED}. Expiration $\rightarrow$ T10. \\
\bottomrule
\end{tabular}
\end{table}
...
\tag{BP-1}
\end{equation}
Equivalently: the signal emitted at $n$ upon entry to \texttt{ESCALATING} reaches $h(n)$ via the immutable parent-pointer chain without requiring any machine actor on the chain to voluntarily forward it. Propagation is enforced by the \fact{} layer's pre-commit hook at each hop, making the path structurally compelled rather than operationally cooperative.
\end{definition}

The necessity of the bypass property rests on three concerns:

\begin{enumerate}\itemsep=0pt
\item[$\textbf{Single point of failure.}$] If the escalation path depended on any machine actor performing a correct action, the failure of that action would cascade into a permanent failure of the accountability guarantee. Any machine actor in the chain becomes a single point of failure for the entire upstream accountability guarantee. The bypass property eliminates all such single points: every node on the chain is structurally forced to propagate, not voluntarily.

\item[$\textbf{Deferral attack.}$] In conventional HITL systems, the agent decides when to invoke human oversight---which means the agent can defer escalation precisely when it is most needed. The bypass property makes deferral structurally impossible: the state machine's transition to \texttt{ESCALATING} is enforced by the \fact{} layer without machine-actor discretion, and propagation is structurally compelled rather than voluntarily performed.

\item[$\textbf{Credential substitution.}$] A machine actor with access to the human anchor's authentication credential could fabricate a resolution, suppressing a genuine escalation. The bypass property prevents this by binding $C_{h(n)}$ exclusively to $h(n)$ and enforcing credential verification at the \fact{} layer on every resolution event.
\end{enumerate}
...
\begin{enumerate}\itemsep=0pt
\item[$\textbf{\tau_{\text{bounds}} ---\ \texttt{BOUNDED} state timeout.}$] The Bounds Engine is allocated $\tau_{\text{bounds}}$ to produce and gain \fact{}-layer approval for a candidate resolution that addresses the trigger condition. If no approval is recorded before $\tau_{\text{bounds}}$ expires, the \fact{} layer forces transition T3 to \texttt{BAIL\_PENDING}. The Bounds Engine may not extend $\tau_{\text{bounds}}$, substitute an alternative resolution path, or suppress the \texttt{bailout\_signal}. Any attempt to write a resolution approval after expiration is rejected by the pre-commit hook. This timeout enforces liveness: the system cannot remain indefinitely in \texttt{BOUNDED} without human resolution.
