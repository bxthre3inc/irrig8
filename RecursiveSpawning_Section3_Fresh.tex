...
Before specifying the Recursive Spawning Protocol, we must precisely characterize the failure mode it targets: \emph{autonomous drift} — the condition in which an agent continues to operate, spawn descendants, and take real-world actions while its delegation chain no longer verifiably connects to any human principal. This is not a behavioral misalignment problem. It is a chain-topology failure: a structural property of the delegation graph, independent of the agent's intentions or capabilities. This section establishes the formal vocabulary for that characterization, defines the drift triad and its constituent sub-modes, identifies the three structural conditions that enable them, catalogs four concrete failure pathways, proves formally that depth limits alone are insufficient, and concludes with a compact formal model that is referenced throughout the remainder of this paper.

% -------------------------------------------------------------------------
\subsection{Formal Characterization}
\label{sec:autonomous-drift-formal}
...
An agent $n$ is \emph{orphaned} at time $t$ if its parent node $\alpha(n)$ satisfies one of the following conditions:
...
Orphaning severs the delegation chain at the parent node. The agent $n$ persists in operational state but is structurally disconnected from its immediate supervisor. Critically, an orphaned agent may retain a verifiable chain to a human principal through other links in the chain — if the grandparent or any ancestor above the decommissioned parent is still active and retains a verifiable connection to $h$, the orphaned agent's accountability connection is intact. Orphaning is a condition of lost \emph{supervisory connection}, not necessarily lost \emph{accountability connection}.

\medskip
\noindent\textbf{Definition 2 (Drifted Agent).}
An agent $n$ is \emph{drifted} at time $t$ if the chain $C(n) = \langle n_0, n_1, \ldots, n \rangle$ obtained by recursively applying $\alpha$ from $n$ does not contain a human principal at its origin. Formally:

\begin{equation}
\mathrm{drifted}(n) \;\iff;\neg\;\exists\; h \in \mathcal{H} : h \text{ is reachable as the terminus of } C(n).
\label{eq:drifted-def}
\end{equation}

A drifted agent may have a living parent. Drift is a property of the chain's \emph{origin}, not of the chain's intermediate connectivity. An agent whose parent is alive and responsive can still be drifted if the chain above the parent traces to a non-human root.

\medskip
\noindent\textbf{Definition 3 (Drift Triad).}
The \emph{drift triad} is the conjunction of three independent conditions that produces the protocol's target failure state:
...
True  & False & True  & Orphaned but authorized — chain broken at parent, human anchor intact above \\
...
A \emph{drift cascade} occurs when a drifted agent $n$ spawns a child $c$, propagating the drift condition hereditarily to all descendants:

\begin{equation}
\mathrm{drifted}(n, t) \;\land;\mathrm{spawned}(n, c, t') \;\land; t' > t \;\Longrightarrow;\mathrm{drifted}(c, t'')\quad \forall\, t'' > t'.
...
We present a compact formal model capturing the structural elements essential to drift analysis. This model is referenced in subsequent sections and used in the protocol correctness proofs.
...
An agent $n$ is in the drift triad (Equation~\ref{eq:drift-triad}) if and only if:
