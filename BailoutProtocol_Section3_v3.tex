...
T5 & \texttt{ESCALATING}           & \texttt{SendToParent} succeeds (propagate; stay)                     & \texttt{ESCALATING} \\
...
\item[\textbf{T9 (HUMAN\_ACKNOWLEDGED $\rightarrow$ IDLE):}] $\sigma_{\text{resolve}}(\texttt{ACK})$ is received from $h(n)$, returning the node to normal operation. Pre-commit: credential verified against $C_{h(n)}$.

\item[\textbf{T10 (HUMAN\_ACKNOWLEDGED $\rightarrow$ TIMEOUT):}] The human acknowledgment timeout $\tau_{\text{human}}$ expires while in \texttt{HUMAN\_ACKNOWLEDGED}. This is a terminal failure state. Pre-commit: a \texttt{timeout-human} entry records the elapsed time and last known state.
\end{enumerate}
...
\midrule
$\tau_{\text{bounds}}$ & $T_{\text{init}}$ & Time granted to the Bounds Engine to produce and gain approval for a candidate resolution during \texttt{BOUNDED}. Expires $\rightarrow$ T3. \\
$\tau_{\text{prop}}$ & $T_{\text{prop}}$ & Time allowed for each \texttt{SendToParent} invocation to receive acknowledgment. Expires $\rightarrow$ T4 or T7. \\
...
$\tau_{\text{human}}$ & $T_{\text{human}}$ & Time allowed for $h(n)$ to acknowledge and issue a directive in \texttt{HUMAN\_ACKNOWLEDGED}. Expires $\rightarrow$ T10. \\
\bottomrule
\end{tabular}
\end{table}
...
\State \textbf{Subroutine:} \Call{SelectPathway}{$P, v$} selects the highest-priority functional
\State \hphantom{\textbf{Subroutine:}}pathway from $P$ for target node $v$, excluding any pathway
\State \hphantom{\textbf{Subroutine:}}marked \texttt{degraded} at $v$.
\end{algorithmic}
\end{algorithm}
...
The bypass property ensures that the Bailout Protocol's accountability guarantee does not depend on any machine actor choosing to comply. In conventional HITL systems, the agent decides when to invoke human oversight---which means the agent can defer escalation precisely when it is most needed. In the \bxthree{} Framework, the state machine's transition to \texttt{ESCALATING} is enforced by the \fact{} layer without machine-actor discretion, and propagation is structurally compelled rather than voluntarily performed.
