# Executive Guide: Where brodiblanco Fits in Agentic
...
You can override any Agentic decision:

```typescript
// Example API (available now)
Agentic.overrideProposal(proposalId, 'approved', 'My reasoning');
Agentic.overrideSprint(sprintId, 'cancel');
Agentic.escalateToMe(agentId, 'Need human judgment');
Agentic.warRoom.tiebreaker(proposalId, 'sideA'); // 3-2 deadlock
```
...
## What You DON'T Do (Agentic Handles)
...
// 1. Stop everything
Agentic.emergencyStop();

// 2. Take direct control of any agent
Agentic.assumeControl(agentId);

// 3. Reset to last known good state
Agentic.rollbackToSnapshot(snapshotId);

// 4. Override consensus
Agentic.warRoom.veto(proposalId, 'My override reasoning');
```
...
*Agentic is your workforce. You are the founder. Erica is your Chief of Staff.*
*March 17, 2026*
